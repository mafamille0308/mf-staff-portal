...
import { showModal, showSelectModal, toast, escapeHtml } from "../ui.js";
import { fetchVisitDetailPolicy } from "./visits_policy.js";
import { upsertCustomerPolicy_ as upsertCustomerPolicy } from "./customer_profile_policy.js";
import { runWithBlocking_ } from "./page_async_helpers.js";

const KEY_LOCATION_OPTIONS = ["顧客", "本部", "担当者", "鍵なし"];

function isFutureVisit_(visit) {
  const s = String(visit?.start_time || "").trim();
  if (!s) return false;
...
export async function confirmKeyLocationBeforeDone({ visitId }) {
  ...
  try {
    detail = await runWithBlocking_(
      ...
      async () => await fetchVisitDetailPolicy(vid, idToken, { include_customer_detail: true })
    );
  } catch (e) {
  ...
  if (isFutureVisit_(visit)) {
    toast({ title: "更新不可", message: "未来の予約は完了にできません。" });
    return false;
  }
  ...
  try {
    const res = await runWithBlocking_(
      ...
      async () => await upsertCustomerPolicy({
...
export async function confirmKeyLocationBeforeBulkDone({ visitIds, visits }) {
  ...
  try {
    details = await runWithBlocking_(
      ...
      async () => {
        const list = [];
        for (const id of ids) {
          const res = await fetchVisitDetailPolicy(id, idToken, { include_customer_detail: true });
          list.push(res);
        }
        return list;
      }
  ...
  const visitHintById = new Map(
    (Array.isArray(visits) ? visits : [])
      .map((v) => [String(v?.visit_id || v?.id || "").trim(), v])
      .filter(([id]) => !!id)
  );
  ...
  for (const detail of details) {
    ...
    const visit = detail.visit || {};
    const visitId = String(visit.visit_id || visit.id || "").trim();
    const visitHint = visitHintById.get(visitId) || null;
    if (isFutureVisit_(visit)) {
      toast({ title: "更新不可", message: "未来の予約は完了にできません。" });
      return false;
    }
    const customer = (detail.customer_detail && detail.customer_detail.customer) ? detail.customer_detail.customer : {};
    const customerId = String(visit.customer_id || visitHint?.customer_id || customer.customer_id || customer.id || "").trim();
    if (!customerId) {
      toast({ title: "更新不可", message: "顧客IDを特定できませんでした。" });
      return false;
    }
    if (!byCustomer.has(customerId)) {
      byCustomer.set(customerId, {
        customerId,
        customerName: String(customer.name || visit.customer_name || visitHint?.customer_name || customerId).trim(),
