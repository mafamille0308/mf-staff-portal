  ...
  const pickOptions = (itemType) => {
    return list
      ...
      .map((r) => {
        ...
        const label = String(r?.variant_name || r?.label || "").trim() || rid;
...
function buildBillingBatchFormHtml_(selected, feeDefaults) {
  ...
  const rowsHtml = list.length
    ? list.map((v) => {
      ...
      const travelRaw = Number(v?.travel_fee_amount || 0) || 0;
      const seasonalRaw = Number(v?.seasonal_fee_amount || 0) || 0;
      const parkingRaw = Number(v?.parking_fee_amount || 0) || 0;
      ...
      const parkingRule = String(v?.parking_fee_rule || "").trim().toLowerCase();
      const parkingDefault = parkingRule === "paid" ? (Number(fd.parking_fee || 0) || 0) : 0;
      const parking = parkingRaw > 0 ? parkingRaw : parkingDefault;
        ...
        <details style="padding:4px 0;">
          ...
          <div style="display:grid; gap:8px; margin-top:8px; padding-left:2px;">
            <button type="button" data-edit="base" data-visit-id="${escapeHtml(vid)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; border:none; background:transparent; color:inherit; padding:6px 0; cursor:pointer;">
              <span>🔄️ ${escapeHtml(String(v?.product_name || v?.service_name || "訪問サービス"))} ${escapeHtml(String(v?.variant_name || ""))}</span>
              <strong data-el="base_amount_${escapeHtml(vid)}">${escapeHtml(formatMoney_(base))}円</strong>
            </button>
            ...
            <button type="button" data-edit="parking" data-visit-id="${escapeHtml(vid)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; border:none; background:transparent; color:inherit; padding:6px 0; cursor:pointer;">
              <span>🔄️ 駐車料金</span>
              <strong data-el="parking_amount_${escapeHtml(vid)}">${escapeHtml(formatMoney_(parking))}円</strong>
            </button>
    ...
    : `<div>-</div>`;
  const first = list[0] || {};
  const keyPickupDefault = String(first?.key_pickup_fee_rule || "").toLowerCase() === "paid" ? (Number(fd.key_pickup_fee || 0) || 0) : 0;
  const keyReturnDefault = String(first?.key_return_fee_rule || "").toLowerCase() === "paid" ? (Number(fd.key_return_fee || 0) || 0) : 0;
  const summaryBase = list.reduce((acc, v) => {
    const product = String(v?.product_name || v?.service_name || "訪問サービス").trim();
    const variant = String(v?.variant_name || "").trim();
    const label = [product, variant].filter(Boolean).join(" ").trim() || "訪問サービス";
    const base = resolveVisitBaseAmount_(v, fd);
    if (!acc[label]) acc[label] = { qty: 0, amount: 0 };
    acc[label].qty += 1;
    ...
    <form data-el="billingBatchForm">
      ...
      <div style="display:grid; gap:8px; margin-bottom:12px;">
        <button type="button" data-edit="key_pickup" style="display:flex; align-items:center; justify-content:space-between; width:100%; border:none; background:transparent; color:inherit; padding:6px 0; cursor:pointer;">
          <span>🔄️ 鍵預かり料金</span>
          <strong data-el="key_pickup_amount">${escapeHtml(formatMoney_(keyPickupDefault))}円</strong>
        </button>
        <button type="button" data-edit="key_return" style="display:flex; align-items:center; justify-content:space-between; width:100%; border:none; background:transparent; color:inherit; padding:6px 0; cursor:pointer;">
          <span>🔄️ 鍵返却料金</span>
          <strong data-el="key_return_amount">${escapeHtml(formatMoney_(keyReturnDefault))}円</strong>
        </button>
      ...
      <div style="margin-bottom:12px;">
        ...
        <div class="row row-between" style="margin-top:6px;">
          <span>請求総額（初期値）</span><strong style="text-align:right;">${escapeHtml(formatMoney_(defaultTotal))}円</strong>
        </div>
        <div style="margin-top:6px; display:grid; gap:4px; opacity:.9;">
          ${summaryLines.map((x) => {
      ...
      <div>
        ...
        <textarea class="input" name="memo" rows="4" placeholder="例: visit_ids: ${escapeHtml(visitIds.join(","))}"></textarea>
      </div>
      </div>
    </form>
  `;
...
function wireBillingBatchFormInteractions_(host, selected, feeDefaults) {
  const list = Array.isArray(selected) ? selected : [];
  const fd = feeDefaults || {};
  const travelOptions = Array.isArray(fd.travel_options) ? fd.travel_options : [];
  const seasonalOptions = Array.isArray(fd.seasonal_options) ? fd.seasonal_options : [];
  ...
  const promptNumber_ = async (title, current) => {
    const val = await openEditor_({
      title,
      bodyHtml: `<input class="input" data-el="num" type="number" min="0" step="1" inputmode="numeric" value="${escapeHtml(String(Math.max(0, Number(current || 0) || 0)))}" />`,
      onSubmit: (root) => {
  ...
  host.querySelectorAll("[data-edit]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      if (type === "parking" && vid) {
        ...
        if (next == null) return;
        setValue_(name, next);
        setText_(`parking_amount_${vid}`, `${formatMoney_(next)}円`);
        return;
      }
      if (type === "travel" && vid) {
      ...
      if (type === "key_pickup") {
        const cur = Number(host.querySelector('[name="key_pickup_fee_amount"]')?.value || 0) || 0;
        const next = await promptNumber_("鍵預かり料金", cur);
        if (next == null) return;
        setValue_("key_pickup_fee_amount", next);
        setText_("key_pickup_amount", `${formatMoney_(next)}円`);
        return;
      }
      if (type === "key_return") {
        const cur = Number(host.querySelector('[name="key_return_fee_amount"]')?.value || 0) || 0;
        const next = await promptNumber_("鍵返却料金", cur);
        if (next == null) return;
        setValue_("key_return_fee_amount", next);
        setText_("key_return_amount", `${formatMoney_(next)}円`);
        return;
      }
      if (type === "discount") {
          ...
          bodyHtml: `
            <div style="display:grid; gap:8px;">
              ...
              <div>
                ...
                <input class="input" data-el="amount" type="number" min="0" step="1" inputmode="numeric" value="${escapeHtml(String(curAmount))}" />
              </div>
            </div>
          `,
          onSubmit: (root) => ({
...
function buildBillingBatchPreviewHtml_(selected, payload) {
  const list = Array.isArray(selected) ? selected : [];
  const visitTitles = list
    .map((v) => String(v?.title || "").trim())
    .filter(Boolean);
  const titlesHtml = visitTitles.length
    ? visitTitles.map((t) => `<div>${escapeHtml(t)}</div>`).join("")
    : `<div>-</div>`;
  const p = payload || {};
  const discountAmount = Number(p?.discount_amount || 0) || 0;
  const discountLabel = String(p?.discount_label || "").trim() || "割引";
  const parkingOverrides = Array.isArray(p?.visit_overrides) ? p.visit_overrides : [];
  const parkingTotal = parkingOverrides.reduce((acc, x) => acc + (Number(x?.parking_fee_amount || 0) || 0), 0);
  return `
    <div class="p" style="margin-bottom:8px;">
    ...
    <div style="margin-bottom:10px;">
      <div><strong>予約タイトル</strong></div>
      <div style="margin-top:6px;">${titlesHtml}</div>
    </div>
    <div>
    ...
    <div style="margin-top:10px;">
      <div><strong>駐車料金合計（入力値）</strong></div>
      <div style="margin-top:6px;">${escapeHtml(formatMoney_(parkingTotal))}円</div>
    </div>
    <div style="margin-top:10px;">
      <div><strong>割引</strong></div>
      <div style="margin-top:6px;">${discountAmount > 0 ? `${escapeHtml(discountLabel)}: -${escapeHtml(formatMoney_(discountAmount))}円` : "なし"}</div>
    </div>
    <div class="p" style="margin-top:8px; opacity:.8;">
      作成後は Square 下書きIDを保存し、各予約の請求ステータスを「請求ドラフト」に更新します。
    </div>
  `;
}
...
function buildBillingBatchPayload_(customerId, selected, formValues) {
  ...
  const visitOverrides = list.map((v) => {
    const vid = String(v?.visit_id || "").trim();
    const travelAmount = parseOptionAmount_(fv[`travel_option_${vid}`]);
    const seasonalAmount = parseOptionAmount_(fv[`seasonal_option_${vid}`]);
    return {
      visit_id: vid,
      base_fee_amount: Math.max(0, Number(fv[`base_fee_amount_${vid}`] || 0) || 0),
      parking_fee_amount: Math.max(0, Number(fv[`parking_fee_amount_${vid}`] || 0) || 0),
      travel_fee_amount: travelAmount,
      seasonal_fee_amount: seasonalAmount,
    };
  }).filter((x) => x.visit_id);
  ...
  const memo = String(fv.memo || "").trim() || `visit_ids: ${visitIds.join(",")}`;
...
function buildSquareInvoiceGuideHtml_(selected, _guide) {
  ...
  list.forEach((v) => {
    const product = String(v?.product_name || v?.service_name || "").trim();
    const variant = String(v?.variant_name || "").trim();
    const baseName = [product, variant].filter(Boolean).join(" ").trim() || "訪問サービス";
    baseMap.set(baseName, (baseMap.get(baseName) || 0) + 1);

    const parkingRule = String(v?.parking_fee_rule || "").trim().toLowerCase();
    if (parkingRule === "paid") parkingCount += 1;
  });

  const first = list[0] || {};
  const keyPickupCount = String(first?.key_pickup_fee_rule || "").trim().toLowerCase() === "paid" ? 1 : 0;
  const keyReturnCount = String(first?.key_return_fee_rule || "").trim().toLowerCase() === "paid" ? 1 : 0;
  const baseLinesHtml = Array.from(baseMap.entries()).map(([label, count]) => `<div>${escapeHtml(`${label} × ${count}`)}</div>`).join("");

  return `
    <div class="p" style="margin-bottom:10px;">Square請求書で生成された下書きを編集して送信してください。</div>
    ${baseLinesHtml || `<div>${escapeHtml("訪問サービス × 0")}</div>`}
    <div style="margin-bottom:10px;">${escapeHtml(`駐車料金 × ${parkingCount}`)}</div>
    <div>${escapeHtml("出張料金 × n")}</div>
    <div style="margin-bottom:10px;">${escapeHtml("繫忙期加算 × n")}</div>
    <div>${escapeHtml(`鍵預かり料金 × ${keyPickupCount}`)}</div>
    <div>${escapeHtml(`鍵返却料金 × ${keyReturnCount}`)}</div>
  `;
}
...
function productBadgeLabel_(v) {
  return String(v?.product_name || v?.service_name || v?.visit_type || "").trim() || "-";
}
...
export async function renderVisitsList(appEl, query) {
  ...
  const runCreateInvoiceDraft_ = async () => {
    ...
    const formValues = await showFormModal({
      title: "請求書ドラフト設定",
      bodyHtml: buildBillingBatchFormHtml_(selected, feeDefaults),
      okText: "確認へ",
      cancelText: "キャンセル",
      formSelector: '[data-el="billingBatchForm"]',
      onOpen: (host) => wireBillingBatchFormInteractions_(host, selected, feeDefaults)
    });
    if (formValues == null) return;

    const payload = buildBillingBatchPayload_(customerIds[0], selected, formValues);

    const ok = await showModal({
      title: "請求書ドラフト確認",
      bodyHtml: buildBillingBatchPreviewHtml_(selected, payload),
      okText: "バッチ作成",
      cancelText: "キャンセル"
    });
    if (!ok) return;
    ...
    try {
      selected.forEach((v) => {
        const vid = String(v?.visit_id || "").trim();
        if (!vid) return;
        const mm = mergeVisitById(visitsAll, vid, { visit_id: vid, billing_status: "invoice_draft" });
        visitsAll = mm.list;
      });
      ...
      const created = await runWithBlocking_(
        ...
        async () => {
          const res = await callGas({ action: "createBillingBatch", ...payload }, idToken2);
      ...
      saveCache_(cacheKey_(state), visitsAll);
      applyAndRender_();
      toast({ title: "作成完了", message: `請求書ドラフト連携バッチを作成しました。${batchId || ""}`.trim() });
      await showModal({
    ...
    if (action === "change-visit-type") {
      ...
      const applyFinal = (u) => {
        ...
        if (vt) {
          ...
          actEl.textContent = productBadgeLabel_(Object.assign({}, v, uu, { visit_type: vt }));
