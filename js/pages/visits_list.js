...
import { inputDiscountByModal_ } from "./invoices_editor_modal.js";
import {
...
function makeBillingFeeContext_(rules) {
  ...
  return {
    ...
    travel_options: pickOptions("travel_fee", "per_visit"),
...
function buildBillingBatchFormHtml_(selected, feeDefaults, customerDefaults, options = {}) {
  ...
  const rowsHtml = list.length
    ? list.map((v) => {
      ...
      const appliedBasePriceRuleId = (!isInactiveVisit && baseRuleAuto) ? basePriceRuleId : "";
      ...
      const seasonalRaw = (adjustmentMode || isInactiveVisit || !hasSeasonalAuto) ? 0 : (Number(v?.seasonal_fee_amount || 0) || 0);
    ...
    : `<div>-</div>`;
  const first = list[0] || {};
  const keyPickupDefault = !adjustmentMode && hasKeyPickupAuto && String(cd?.key_pickup_fee_rule || first?.key_pickup_fee_rule || "").toLowerCase() === "paid" ? (Number(fd.key_pickup_fee || 0) || 0) : 0;
  const keyReturnDefault = !adjustmentMode && hasKeyReturnAuto && String(cd?.key_return_fee_rule || first?.key_return_fee_rule || "").toLowerCase() === "paid" ? (Number(fd.key_return_fee || 0) || 0) : 0;
  ...
  list.forEach((v) => {
    ...
    const basePriceRuleId = String(v?.price_rule_id || "").trim();
    const parkingAmount = hasParkingAuto ? Math.max(0, Number(cd?.parking_fee_amount || 0) || 0) : 0;
    const travelRaw = (!legacyMode && hasTravelAuto) ? Math.max(0, Number(cd?.travel_fee_amount || 0) || 0) : 0;
...
function wireBillingBatchFormInteractions_(host, selected, feeDefaults, options = {}) {
  ...
  const pickAdjustmentLineByModal_ = async () => {
    const groups = [
      ...
      { key: "travel_fee", label: "交通費", options: travelOptions },
  ...
  const updateTotals_ = () => {
    ...
    list.forEach((v) => {
      ...
      extraLines.forEach((line) => {
        addLine_(line.label, line.quantity * line.unit_price, line.quantity, line.price_rule_id);
      });
