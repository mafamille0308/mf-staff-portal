...
function squareConfigMissingToastMessage_() {
  return {
    ...
    action: "設定後、もう一度「既存請求書に紐づけ」を実行してください。",
  };
}
...
function buildBillingBatchFormHtml_(selected, feeDefaults, customerDefaults, options = {}) {
  ...
  const rowsHtml = list.length
    ? list.map((v) => {
      ...
      const legacyTravelDefaultAmount = (!cancellationMode && !adjustmentMode && legacyMode)
        ? Math.max(0, Number(cd?.travel_fee_amount || 0) || 0)
        : 0;
  ...
  list.forEach((v) => {
    ...
    const base = resolveVisitBaseAmount_(v, fd);
    const basePriceRuleId = String(v?.price_rule_id || "").trim();
    const parkingAmount = cancellationMode ? 0 : Math.max(0, Number(cd?.parking_fee_amount || 0) || 0);
    const travelRaw = cancellationMode || legacyMode ? 0 : Math.max(0, Number(cd?.travel_fee_amount || 0) || 0);
    const seasonalRaw = Number(v?.seasonal_fee_amount || 0) || 0;
    const legacyTravelAmount = !cancellationMode && legacyMode ? Math.max(0, Number(cd?.travel_fee_amount || 0) || 0) : 0;
...
function buildBillingBatchPayload_(customerId, selected, formValues, options = {}) {
  ...
  return {
    ...
    extra_lines: adjustmentMode
      ? adjustmentLines
      : extraLines.concat(legacyFixedLines, residualExtraLines, sourceExtraLines),
...
export async function renderVisitsList(appEl, query) {
  ...
  const runCreateInvoiceDraft_ = async (options = {}) => {
    ...
    const canceledVisitIdsOpt = Array.isArray(options.canceled_visit_ids)
      ...
      : [];
    if (cancellationRate === 0 && sourceBatchId && canceledVisitIdsOpt.length) {
      const canceledSet = new Set(canceledVisitIdsOpt);
      ids = ids.filter((id) => id && !canceledSet.has(id));
      if (!ids.length) {
        toast({ title: "対象なし", message: "キャンセル後に請求書へ残す予約がありません。" });
        return;
      }
    }
    ...
    if (sourceBatchId) {
      try {
        ...
        const shouldRestoreSourceDraft = !cancellationMode && !adjustmentMode && cancellationRate === 0 && canceledVisitIdsOpt.length > 0;
        const shouldBuildSourceSeed = !cancellationMode && !adjustmentMode && (sourceVisitIdsOpt.length > 0 || shouldRestoreSourceDraft);
        ...
        } else if (shouldRestoreSourceDraft && sourceSeed.visits.length) {
          sourceRestoreSeed = sourceSeed;
        }
    ...
    while (true) {
      ...
      if (!cancellationMode && !adjustmentMode && sourceReissueSeed.visits.length) {
      ...
      const ok = await showModal({
        ...
        bodyHtml: buildBillingBatchPreviewHtml_(previewSelected, payloadForPreview, feeDefaults, {
          cancellation_rate: cancellationRate,
        }),
  ...
  const runBulkEdit_ = async (pickedItemOpt = "", idsOverride = null) => {
    ...
    if (item === "is_active" && fields && fields.is_active === false) {
      ...
      for (const vid of policyIds) {
        ...
        const previewStatusRaw = String(preview?.billing_status || "").trim().toLowerCase();
        const isDraftPreviewStatus = ["draft", "invoice_draft", "pending", "unpaid"].includes(previewStatusRaw);
        const batchId = String(preview?.source_batch_id || "").trim();
        ...
        if (useDraftFlow && previewStatus === "unbilled") {
          ...
          const shouldReuseDraftConfirm = !!batchId
            && !isFullBatchCancel
            && (isDraftPreviewStatus || (useDraftFlow && previewStatus === "billed"));
          ...
          for (const plan of cancellationPlans) {
            ...
            try {
              ...
              if (!skipDraftForFullBatchZero && previewStatus !== "paid" && done?.next_action === "create_cancellation_invoice") {
                ...
                const remainingIdsRaw = Array.isArray(done?.remaining_active_visit_ids)
                  ? done.remaining_active_visit_ids.map((x) => String(x || "").trim()).filter(Boolean)
                  : [];
                const remainingIds = remainingIdsRaw.filter((id) => id && !selectedSet.has(id));
