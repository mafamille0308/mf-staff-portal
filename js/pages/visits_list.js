...
import { buildBillingLineGroupKey_ } from "./billing_line_group_key.js";
...
import { inputCancellationFeeRateByModal_, inputDiscountByModal_ } from "./invoices_editor_modal.js";
import {
...
function priceRuleDisplayLabel_(r, fallbackId = "") {
  const product = String(r?.product_name || "").trim();
  const variant = String(r?.variant_name || "").trim();
  const joined = [product, variant].filter(Boolean).join(" ").trim();
  const raw = String(r?.label || "").trim();
  if (product && variant && product === variant && (!raw || raw === joined)) return product;
  return raw || joined || String(fallbackId || "").trim();
}

function makeBillingFeeContext_(rules) {
  const list = (Array.isArray(rules) ? rules : []).filter((r) => isSelectablePriceRule_(r));
  const toRuleOption_ = (r) => {
    const rid = String(r?.price_rule_id || "").trim();
    const label = priceRuleDisplayLabel_(r, rid) || rid;
    ...
    return {
      ...
      category_key: String(r?.category_key || "").trim(),
      store_category: String(r?.store_category || "").trim(),
      unit_label: String(r?.unit_label || "").trim(),
      invoice_inclusion: String(r?.invoice_inclusion || "").trim(),
      price_mode: String(r?.price_mode || "").trim(),
  ...
  const hasInclusion_ = (r, inclusion) => String(r?.invoice_inclusion || "").trim() === inclusion;
  const pickOptions = (itemType, inclusion = "") => {
    return list
      .filter((r) => String(r?.item_type || "").trim() === itemType && r?.is_active !== false)
      .filter((r) => !inclusion || hasInclusion_(r, inclusion))
      .sort((a, b) => (Number(a?.display_order || 0) || 0) - (Number(b?.display_order || 0) || 0))
      .map(toRuleOption_);
  };
  const pickCategoryOptions = (categoryKey, itemType, inclusion = "") => {
    return list
      .filter((r) => r?.is_active !== false)
      .filter((r) => String(r?.category_key || "").trim() === categoryKey)
      .filter((r) => !itemType || String(r?.item_type || "").trim() === itemType)
      .filter((r) => !inclusion || hasInclusion_(r, inclusion))
      .sort((a, b) => (Number(a?.display_order || 0) || 0) - (Number(b?.display_order || 0) || 0))
      .map(toRuleOption_);
  };
  const visitBaseRules = list
    ...
    .filter((r) => hasInclusion_(r, "per_visit"))
    .sort((a, b) => (Number(a?.display_order || 0) || 0) - (Number(b?.display_order || 0) || 0));
  ...
  const reimbursementOptions = list
    ...
    .sort((a, b) => (Number(a?.display_order || 0) || 0) - (Number(b?.display_order || 0) || 0))
    .map(toRuleOption_);
  const manualOptions = list
    .filter((r) => r?.is_active !== false)
    .filter((r) => {
      const itemType = String(r?.item_type || "").trim();
      const categoryKey = String(r?.category_key || "").trim();
      const inclusion = String(r?.invoice_inclusion || "").trim();
      if (itemType === "discount" || categoryKey === "adjustment") return false;
      if (itemType === "visit_addon" || categoryKey === "visit_addon") return false;
      if (inclusion === "manual") return true;
      if (!inclusion && ["merchandise", "reimbursement", "cancellation_fee", "overtime_fee"].includes(itemType)) return true;
      return false;
    })
    .sort((a, b) => (Number(a?.display_order || 0) || 0) - (Number(b?.display_order || 0) || 0))
    .map(toRuleOption_);
  return {
    ...
    parking_options: pickOptions("parking_fee", "per_visit"),
    key_pickup_fee: pickAmount("key_pickup_fee"),
    key_return_fee: pickAmount("key_return_fee"),
    key_pickup_options: pickOptions("key_pickup_fee", "per_invoice"),
    key_return_options: pickOptions("key_return_fee", "per_invoice"),
    visit_addon_options: pickOptions("visit_addon", "manual"),
    travel_options: list
      .filter((r) => r?.is_active !== false && String(r?.item_type || "").trim() === "travel_fee")
      .filter((r) => {
        const categoryKey = String(r?.category_key || "").trim();
        return !categoryKey || categoryKey === "business_trip_fee";
      })
      .filter((r) => hasInclusion_(r, "per_visit"))
      .sort((a, b) => (Number(a?.display_order || 0) || 0) - (Number(b?.display_order || 0) || 0))
      .map(toRuleOption_),
    transport_options: pickCategoryOptions("travel_fee", "travel_fee", "per_visit"),
    seasonal_options: pickOptions("seasonal_fee", "per_visit"),
...
function buildBillingBatchFormHtml_(selected, feeDefaults, customerDefaults, options = {}) {
  ...
  const visitAddonOptions = Array.isArray(fd.visit_addon_options) ? fd.visit_addon_options : [];
  const travelOptions = Array.isArray(fd.travel_options) ? fd.travel_options : [];
  const transportOptions = Array.isArray(fd.transport_options) ? fd.transport_options : [];
  const seasonalOptions = Array.isArray(fd.seasonal_options) ? fd.seasonal_options : [];
  const hasParkingAuto = parkingOptions.length > 0;
  const hasVisitAddonOptions = visitAddonOptions.length > 0;
  const hasTravelAuto = travelOptions.length > 0;
  const hasTransportAuto = transportOptions.length > 0;
  const hasSeasonalAuto = seasonalOptions.length > 0;
  ...
  const hasKeyPickupAuto = keyPickupOptions.length > 0;
  const hasKeyReturnAuto = keyReturnOptions.length > 0;
  ...
  registerDisplayOrder_(visitAddonOptions);
  registerDisplayOrder_(parkingOptions);
  registerDisplayOrder_(travelOptions);
  registerDisplayOrder_(transportOptions);
  ...
  const resolveCustomerFeeDefault_ = (categoryKey, feeRole, opts) => {
    const settings = Array.isArray(cd?.customer_fee_settings) ? cd.customer_fee_settings : [];
    const setting = settings.find((row) => (
      String(row?.category_key || "").trim() === categoryKey
      && String(row?.fee_role || "").trim() === feeRole
    ));
    if (!setting) {
      return { amount: 0, price_rule_id: "" };
    }
    const rid = String(setting?.price_rule_id || "").trim();
    const rule = (Array.isArray(opts) ? opts : []).find((x) => String(x?.price_rule_id || "").trim() === rid);
    if (!rule) return { amount: 0, price_rule_id: "" };
    const mode = String(setting?.value_mode || "").trim();
    const amount = mode === "fixed_amount"
      ? Math.max(0, Number(setting?.amount_override || 0) || 0)
      : Math.max(0, Number(rule?.amount || 0) || 0);
    return { amount, price_rule_id: rid };
  };
  ...
  const rowsHtml = list.length
    ? list.map((v) => {
      ...
      const requiresBillingAdjustment = v?.invoice_reconcile_required === true || String(v?.invoice_reconcile_required || "").toLowerCase() === "true";
      const showCancellationFeeModeRow = !adjustmentMode && (isInactiveVisit || requiresBillingAdjustment);
      const productLabel = resolveVisitServiceLabel_(v, fd);
      const basePriceRuleId = String(v?.price_rule_id || "").trim();
      const baseRuleAuto = (Array.isArray(fd.visit_base_rules) ? fd.visit_base_rules : [])
        .some((r) => String(r?.price_rule_id || "").trim() === basePriceRuleId);
      const appliedBasePriceRuleId = baseRuleAuto ? basePriceRuleId : "";
      const base = resolveVisitBaseAmount_(v, fd);
      const baseInitialAmount = (adjustmentMode || !baseRuleAuto) ? 0 : base;
      const baseInitialRuleId = (adjustmentMode || !baseRuleAuto) ? "" : appliedBasePriceRuleId;
      const parkingDefault = resolveCustomerFeeDefault_("parking_fee", "parking", parkingOptions);
      const businessTripDefault = resolveCustomerFeeDefault_("business_trip_fee", "business_trip", travelOptions);
      const transportDefault = resolveCustomerFeeDefault_("travel_fee", "travel", transportOptions);
      const travelRaw = adjustmentMode
        ...
        : !hasTravelAuto
          ? 0
        : businessTripDefault.amount;
      const transportRaw = (adjustmentMode || legacyMode || !hasTransportAuto) ? 0 : transportDefault.amount;
      const seasonalRaw = (adjustmentMode || !hasSeasonalAuto) ? 0 : (Number(v?.seasonal_fee_amount || 0) || 0);
      const parkingRaw = adjustmentMode
        ? 0
        : !hasParkingAuto
          ? 0
        : parkingDefault.amount;
      const travelRuleId = businessTripDefault.price_rule_id || findRuleForAmount_(travelOptions, travelRaw);
      const transportRuleId = transportDefault.price_rule_id;
      ...
      const rowSubtotal = adjustmentMode
        ? Math.max(0, parking)
        : Math.max(0, base + parking + travelAmount + transportRaw + seasonalAmount);
      const hasBaseSelection = !adjustmentMode && !!appliedBasePriceRuleId;
      ...
      const legacyTravelDefaultAmount = (!adjustmentMode && legacyMode)
        ? businessTripDefault.amount
        : 0;
      ...
      return `
        <details class="details-compact">
          ...
          <div class="section-grid-pl2">
            <input type="hidden" name="billing_line_mode_${escapeHtml(vid)}" value="normal" />
            <input type="hidden" name="cancellation_fee_rate_type_${escapeHtml(vid)}" value="" />
            <input type="hidden" name="cancellation_fee_source_amount_${escapeHtml(vid)}" value="0" />
            <input type="hidden" name="cancellation_fee_final_amount_${escapeHtml(vid)}" value="0" />
            <input type="hidden" name="cancellation_fee_manual_reason_${escapeHtml(vid)}" value="" />
            <input type="hidden" name="cancellation_fee_price_rule_id_${escapeHtml(vid)}" value="manual:cancellation_fee" />
            <input type="hidden" name="cancellation_fee_label_${escapeHtml(vid)}" value="キャンセル料金" />

            ${showCancellationFeeModeRow ? `<button type="button" data-edit="billing_line_mode" data-visit-id="${escapeHtml(vid)}" class="fee-edit-btn">
              <span>🔄️ <span data-el="billing_line_mode_label_${escapeHtml(vid)}">キャンセル料金に切り替え</span></span>
              <strong data-el="billing_line_mode_amount_${escapeHtml(vid)}"></strong>
            </button>` : ``}

            ${(!adjustmentMode && (isInactiveVisit || baseRuleAuto)) ? `<button type="button" data-edit="base" data-visit-id="${escapeHtml(vid)}" class="fee-edit-btn">
            ...
            ${(!adjustmentMode && !legacyMode) ? `<div data-el="visit_option_lines_${escapeHtml(vid)}" class="grid-6"></div>` : ``}
            ${(!adjustmentMode && !legacyMode && hasVisitAddonOptions) ? `
            <button type="button" data-edit="visit_option" data-visit-id="${escapeHtml(vid)}" class="fee-edit-btn">
              <span>➕ 予約オプション追加</span>
              <strong data-el="visit_option_label_${escapeHtml(vid)}">未設定</strong>
            </button>
            ` : ``}

            ${(!adjustmentMode && hasParkingAuto) ? `<button type="button" data-edit="parking" data-visit-id="${escapeHtml(vid)}" class="fee-edit-btn">
              <span>🔄️ 駐車料金</span>
            ...
            ${(!adjustmentMode && hasTravelAuto) ? `<button type="button" data-edit="travel" data-visit-id="${escapeHtml(vid)}" class="fee-edit-btn">
              <span>🔄️ 出張料金</span>
            ...
            ${(!adjustmentMode && hasTransportAuto) ? `<button type="button" data-edit="transport" data-visit-id="${escapeHtml(vid)}" class="fee-edit-btn">
              <span>🔄️ 交通費</span>
              <strong data-el="transport_amount_${escapeHtml(vid)}">${transportRaw > 0 ? `${escapeHtml(formatMoney_(transportRaw))}円` : "未選択"}</strong>
            </button>` : ``}
            <input type="hidden" name="transport_option_${escapeHtml(vid)}" value="${escapeHtml(adjustmentMode ? "" : (transportRuleId && transportRaw > 0 ? `${transportRuleId}|${transportRaw}` : ""))}" />

            ${(!adjustmentMode && hasSeasonalAuto) ? `<button type="button" data-edit="seasonal" data-visit-id="${escapeHtml(vid)}" class="fee-edit-btn">
              <span>🔄️ 繁忙期加算</span>
            ...
            <button type="button" data-edit="adjustment_item" data-visit-id="${escapeHtml(vid)}" class="fee-edit-btn">
              <span>➕ 追加請求項目を追加</span>
    ...
    : `<div>-</div>`;
  const keyPickupSetting = resolveCustomerFeeDefault_("key_fee", "key_pickup", keyPickupOptions);
  const keyReturnSetting = resolveCustomerFeeDefault_("key_fee", "key_return", keyReturnOptions);
  const keyPickupDefault = !adjustmentMode && hasKeyPickupAuto ? keyPickupSetting.amount : 0;
  const keyReturnDefault = !adjustmentMode && hasKeyReturnAuto ? keyReturnSetting.amount : 0;
  const keyPickupDefaultRuleId = keyPickupDefault > 0 ? keyPickupSetting.price_rule_id : "";
  const keyReturnDefaultRuleId = keyReturnDefault > 0 ? keyReturnSetting.price_rule_id : "";
  const discountDefaultRuleId = "";
  const summaryBase = {};
  ...
  list.forEach((v) => {
    ...
    const basePriceRuleId = String(v?.price_rule_id || "").trim();
    const parkingDefault = resolveCustomerFeeDefault_("parking_fee", "parking", parkingOptions);
    const parkingAmount = hasParkingAuto ? parkingDefault.amount : 0;
    const businessTripDefault = resolveCustomerFeeDefault_("business_trip_fee", "business_trip", travelOptions);
    const transportDefault = resolveCustomerFeeDefault_("travel_fee", "travel", transportOptions);
    const travelRaw = (!legacyMode && hasTravelAuto) ? businessTripDefault.amount : 0;
    const transportRaw = (!legacyMode && hasTransportAuto) ? transportDefault.amount : 0;
    const seasonalRaw = hasSeasonalAuto ? (Number(v?.seasonal_fee_amount || 0) || 0) : 0;
    const legacyTravelAmount = legacyMode ? businessTripDefault.amount : 0;
    ...
    addSummaryLine_("交通費", transportRaw, transportRuleId);
  ...
  return `
    <form data-el="billingBatchForm">
      ...
      <div class="grid-8 mb-12">
        ${hasKeyPickupAuto ? `<button type="button" data-edit="key_pickup" class="fee-edit-btn">
          <span>🔄️ 鍵預かり料金</span>
          <strong data-el="key_pickup_amount">${escapeHtml(amountText_(keyPickupDefault))}</strong>
        </button>` : ``}
        ${hasKeyReturnAuto ? `<button type="button" data-edit="key_return" class="fee-edit-btn">
          <span>🔄️ 鍵返却料金</span>
...
function wireBillingBatchFormInteractions_(host, selected, feeDefaults, options = {}) {
  ...
  const keyReturnOptions = Array.isArray(fd.key_return_options) ? fd.key_return_options : [];
  const merchandiseOptions = Array.isArray(fd.merchandise_options) ? fd.merchandise_options : [];
  const manualOptions = Array.isArray(fd.manual_options) ? fd.manual_options : [];
  const overtimeOptions = Array.isArray(fd.overtime_options) ? fd.overtime_options : [];
  const reimbursementOptions = Array.isArray(fd.reimbursement_options) ? fd.reimbursement_options : [];
  const adjustmentMode = options?.adjustment_mode === true;
  const legacyMode = options?.legacy_mode === true;
  ...
  const legacyToppingOptions = legacyBuckets.topping;
  const discountOptions = Array.isArray(fd.discount_options) ? fd.discount_options : [];
  const uniqueOptionsByRuleId_ = (optionsList) => {
    const seen = new Set();
    return (Array.isArray(optionsList) ? optionsList : []).filter((option) => {
      const rid = String(option?.price_rule_id || "").trim();
  ...
  const readExtraLinesForVisit_ = (vid) => {
    return Array.from(host.querySelectorAll(`[data-extra-line][data-visit-id="${vid}"]`)).map((row) => {
      ...
      const priceRuleId = String(host.querySelector(`[name="extra_line_price_rule_id_${vid}_${key}"]`)?.value || "").trim();
      ...
      return { key, label, quantity: qty, unit_price: amount, price_rule_id: priceRuleId, item_type: itemType, note };
    }).filter(Boolean);
  };
  const normalFeeEditTypes_ = new Set([
    "base",
    "visit_option",
    "parking",
    "travel",
  ...
  const getBillingLineMode_ = (vid) => String(host.querySelector(`[name="billing_line_mode_${vid}"]`)?.value || "normal").trim() === "cancellation_fee"
    ? "cancellation_fee"
    : "normal";
  const computeVisitNormalSourceAmount_ = (vid) => {
    ...
    const transport = parseOptionAmount_(host.querySelector(`[name="transport_option_${vid}"]`)?.value || "");
    const seasonal = parseOptionAmount_(host.querySelector(`[name="seasonal_option_${vid}"]`)?.value || "");
    const extraTotal = readExtraLinesForVisit_(vid)
      .filter((line) => String(line?.item_type || "").trim() !== "cancellation_fee")
      .reduce((acc, line) => acc + ((Number(line?.quantity || 0) || 0) * (Number(line?.unit_price || 0) || 0)), 0);
    ...
    return Math.max(0, base + parking + travel + transport + seasonal + extraTotal
      + (legacyHeadcountQty * legacyHeadcountAmount)
      + legacyTravelAmount
      + (legacyToppingQty * legacyToppingAmount));
  };
  const applyBillingLineModeUi_ = (vid) => {
    const cancellationMode = getBillingLineMode_(vid) === "cancellation_fee";
    host.querySelectorAll(`[data-visit-id="${vid}"][data-edit]`).forEach((btn) => {
      const editType = String(btn.getAttribute("data-edit") || "").trim();
      if (!normalFeeEditTypes_.has(editType)) return;
      btn.disabled = cancellationMode;
      btn.style.opacity = cancellationMode ? ".45" : "";
      btn.style.cursor = cancellationMode ? "not-allowed" : "";
    });
    const rateType = String(host.querySelector(`[name="cancellation_fee_rate_type_${vid}"]`)?.value || "").trim();
    const finalAmount = Math.max(0, Number(host.querySelector(`[name="cancellation_fee_final_amount_${vid}"]`)?.value || 0) || 0);
    const rateLabel = rateType === "manual" ? "自由入力" : `${rateType || "未設定"}%`;
    setText_(`billing_line_mode_label_${vid}`, cancellationMode ? `キャンセル料金 ${rateLabel}` : "キャンセル料金に切り替え");
    setText_(`billing_line_mode_amount_${vid}`, cancellationMode && finalAmount > 0 ? `${formatMoney_(finalAmount)}円` : "");
  };
  const pickCancellationFeeMode_ = async (vid) => {
    const normalSourceAmount = computeVisitNormalSourceAmount_(vid);
    const storedSourceAmount = Math.max(0, Number(host.querySelector(`[name="cancellation_fee_source_amount_${vid}"]`)?.value || 0) || 0);
    const sourceAmount = normalSourceAmount > 0 ? normalSourceAmount : storedSourceAmount;
    const currentRate = String(host.querySelector(`[name="cancellation_fee_rate_type_${vid}"]`)?.value || "50").trim() || "50";
    const currentFinal = Math.max(0, Number(host.querySelector(`[name="cancellation_fee_final_amount_${vid}"]`)?.value || 0) || 0);
    const currentReason = String(host.querySelector(`[name="cancellation_fee_manual_reason_${vid}"]`)?.value || "").trim();
    const val = await inputCancellationFeeRateByModal_({
      rate_type: currentRate,
      source_amount: sourceAmount,
      final_amount: currentFinal,
      manual_reason: currentReason,
    });
    return val;
  };
  const countVisitOptionLinesForVisit_ = (vid) => {
    return readExtraLinesForVisit_(vid).filter((line) => {
      return String(line?.item_type || "").trim() === "visit_addon";
    }).length;
  };
  const countManualExtraLinesForVisit_ = (vid) => {
    return readExtraLinesForVisit_(vid).filter((line) => {
      return String(line?.item_type || "").trim() !== "visit_addon";
    }).length;
  };
  const addExtraLineRow_ = (vid, line) => {
    ...
    const targetKey = (!adjustmentMode && itemType === "visit_addon")
      ? `visit_option_lines_${vid}`
      : `extra_lines_${vid}`;
    const container = host.querySelector(`[data-el="${targetKey}"]`);
    if (!container) return;
    ...
    row.innerHTML = `
      ...
      <input type="hidden" name="extra_line_price_rule_id_${escapeHtml(vid)}_${escapeHtml(key)}" value="${escapeHtml(priceRuleId)}" />
  ...
  const openEditor_ = ({ title, bodyHtml, okText = "決定", cancelText = "キャンセル", onSubmit, onOpen }) => {
  ...
  const pickExtraLineByModal_ = async (preset = {}) => {
    ...
    const catalogOptions = uniqueOptionsByRuleId_(manualOptions);
    const defaultCatalogMode = !presetLabel && catalogOptions.length > 0;
    const html = `
      <div class="grid-8">
        <div>
          <div class="label-strong">追加方法</div>
          <select class="select w-100" data-el="mode">
            <option value="catalog" ${defaultCatalogMode ? "selected" : ""}>料金マスタから選択</option>
            <option value="free" ${defaultCatalogMode ? "" : "selected"}>自由入力で追加</option>
          </select>
        </div>
        <div data-mode-section="catalog">
          <div class="label-strong">料金マスタ</div>
          <select class="select w-100" data-el="rule">
            <option value="">選択してください</option>
            ${catalogOptions.map((o) => {
              const amountText = String(o?.customer_fee_value_mode || "") === "manual_at_invoice"
                ? "金額入力"
                : `${formatMoney_(o.amount)}円`;
              return `<option value="${escapeHtml(String(o.price_rule_id || ""))}">${escapeHtml(String(o.label || ""))} (${escapeHtml(amountText)})</option>`;
            }).join("")}
          </select>
        </div>
        <div data-mode-section="free">
        ...
        <div data-mode-section="catalog free">
    ...
    return openEditor_({
      ...
      onOpen: (root) => {
        const sync_ = () => {
          const mode = String(root.querySelector('[data-el="mode"]')?.value || "").trim();
          root.querySelectorAll('[data-mode-section]').forEach((el) => {
            const section = String(el.getAttribute("data-mode-section") || "");
            el.style.display = section.split(/\s+/).includes(mode) ? "" : "none";
          });
        };
        const fillCatalogAmount_ = () => {
          const rid = String(root.querySelector('[data-el="rule"]')?.value || "").trim();
          const chosen = catalogOptions.find((o) => String(o?.price_rule_id || "").trim() === rid);
          const amountEl = root.querySelector('[data-el="amount"]');
          if (chosen && amountEl && !String(amountEl.value || "").trim() && String(chosen.customer_fee_value_mode || "") !== "manual_at_invoice") {
            amountEl.value = String(Math.max(0, Number(chosen.amount || 0) || 0));
          }
        };
        root.querySelector('[data-el="rule"]')?.addEventListener("change", () => {
          const amountEl = root.querySelector('[data-el="amount"]');
          if (amountEl) amountEl.value = "";
          fillCatalogAmount_();
        });
        root.querySelector('[data-el="mode"]')?.addEventListener("change", sync_);
        sync_();
        fillCatalogAmount_();
      },
      onSubmit: (root) => {
        ...
        if (mode === "catalog") {
          ...
          const lineAmount = amount > 0 ? amount : Math.max(0, Number(chosen.amount || 0) || 0);
          if (!(lineAmount > 0)) return null;
          return {
            label: String(chosen.label || rid).trim(),
            quantity: qty,
            unit_price: lineAmount,
            price_rule_id: rid,
            item_type: String(chosen.item_type || "").trim(),
            note: String(chosen.note || "").trim(),
          };
        }
  ...
  const pickVisitOptionLineByModal_ = async () => {
    const catalogOptions = uniqueOptionsByRuleId_(visitAddonOptions);
    if (!catalogOptions.length) {
      window.alert("予約オプションに使える商品がありません。商品管理で追加オプションを作成してください。");
      return null;
    }
    const html = `
      <div class="grid-8">
        <div>
          <div class="label-strong">予約オプション</div>
          <select class="select w-100" data-el="rule">
            <option value="">選択してください</option>
            ${catalogOptions.map((o) => `<option value="${escapeHtml(String(o.price_rule_id || ""))}">${escapeHtml(String(o.label || ""))} (${escapeHtml(formatMoney_(o.amount))}円)</option>`).join("")}
          </select>
        </div>
        <div>
          <div class="label-strong">数量</div>
    ...
    return openEditor_({
      ...
      onSubmit: (root) => {
        ...
        return {
          ...
          unit_price: amount,
          price_rule_id: rid,
          item_type: "visit_addon",
          note: String(chosen.note || "").trim(),
        };
      }
    });
  };
  const pickAdjustmentLineByModal_ = async () => {
    const groups = [
      { key: "visit_addon", label: "追加オプション", options: visitAddonOptions },
      { key: "parking_fee", label: "駐車料金", options: parkingOptions },
      { key: "travel_fee", label: "交通費", options: transportOptions },
      { key: "business_trip_fee", label: "出張料金", options: travelOptions },
      { key: "seasonal_fee", label: "繁忙期加算", options: seasonalOptions },
      { key: "extension_fee", label: "延長料金", options: overtimeOptions },
      { key: "cancellation_fee", label: "キャンセル料金", options: manualOptions },
      { key: "manual_item", label: "その他", options: manualOptions },
      { key: "free_manual", label: "自由入力", options: [] },
    ].map((group) => Object.assign({}, group, {
      options: (Array.isArray(group.options) ? group.options : [])
        .filter((option) => {
          const itemType = String(option?.item_type || "").trim();
          const categoryKey = String(option?.category_key || "").trim();
          if (group.key === "manual_item") {
            if (categoryKey === "manual_item") return !["cancellation_fee", "discount"].includes(itemType);
            return !categoryKey && ["merchandise", "reimbursement"].includes(itemType);
          }
          if (group.key === "free_manual") return true;
          if (group.key === "visit_addon") return categoryKey === "visit_addon";
          if (group.key === "travel_fee") return categoryKey === "travel_fee";
          if (group.key === "business_trip_fee") return categoryKey === "business_trip_fee" || (!categoryKey && itemType === "travel_fee");
          if (group.key === "extension_fee") return categoryKey === "extension_fee" || itemType === "overtime_fee";
          if (group.key === "cancellation_fee") return categoryKey === "cancellation_fee" || itemType === "cancellation_fee";
          return categoryKey === group.key || itemType === group.key;
        })
    })).filter((group) => group.key === "free_manual" || group.options.length);
    if (!groups.length) {
      window.alert("追加請求に使える料金マスタがありません。商品管理で追加してください。");
      return null;
    }
    const initialGroup = groups[0] || { key: "", options: [] };
    const optionHtml_ = (options) => options
      .map((o) => `<option value="${escapeHtml(String(o.price_rule_id || ""))}">${escapeHtml(String(o.label || ""))} (${escapeHtml(formatMoney_(o.amount))}円)</option>`)
      .join("");
    const html = `
      <div class="grid-8">
        <div>
          <div class="label-strong">カテゴリ選択</div>
          <select class="select w-100" data-el="group">
        ...
        <div data-mode-section="catalog">
          ...
          <select class="select w-100" data-el="rule">
            ${optionHtml_(initialGroup.options)}
          </select>
        </div>
        <div data-mode-section="free">
          <div class="label-strong">表示名</div>
          <input class="input" data-el="label" type="text" placeholder="例：追加調整" />
        </div>
        <div>
          <div class="label-strong">単価</div>
          <input class="input text-right" data-el="amount" type="number" min="0" step="1" inputmode="numeric" />
        </div>
        <div>
          <div class="label-strong">数量</div>
          <input class="input text-right" data-el="qty" type="number" min="1" step="1" inputmode="numeric" value="1" />
        </div>
      </div>
    `;
    return openEditor_({
      ...
      bodyHtml: html,
      onOpen: (root) => {
        const groupEl = root.querySelector('[data-el="group"]');
        const ruleEl = root.querySelector('[data-el="rule"]');
        const amountEl = root.querySelector('[data-el="amount"]');
        const selectedGroup_ = () => groups.find((group) => group.key === String(groupEl?.value || "").trim()) || initialGroup;
        const syncRules_ = () => {
          const group = selectedGroup_();
          const isFree = group.key === "free_manual";
          root.querySelectorAll('[data-mode-section="catalog"]').forEach((el) => {
            el.hidden = isFree;
            el.classList.toggle("hidden", isFree);
          });
          root.querySelectorAll('[data-mode-section="free"]').forEach((el) => {
            el.hidden = !isFree;
            el.classList.toggle("hidden", !isFree);
          });
          if (ruleEl) {
            ruleEl.innerHTML = optionHtml_(group.options);
            ruleEl.disabled = isFree;
          }
          syncAmount_();
        };
        const syncAmount_ = () => {
          const group = selectedGroup_();
          if (group.key === "free_manual") {
            if (amountEl) amountEl.value = "";
            return;
          }
          const rid = String(ruleEl?.value || "").trim();
          const picked = groups.flatMap((group) => group.options).find((o) => String(o?.price_rule_id || "").trim() === rid);
          if (amountEl) amountEl.value = picked ? String(Math.max(0, Number(picked.amount || 0) || 0)) : "";
        };
        groupEl?.addEventListener("change", syncRules_);
        ruleEl?.addEventListener("change", syncAmount_);
        syncRules_();
      },
      onSubmit: (root) => {
        const group = groups.find((x) => x.key === String(root.querySelector('[data-el="group"]')?.value || "").trim()) || initialGroup;
        const amount = Math.max(0, Number(root.querySelector('[data-el="amount"]')?.value || 0) || 0);
        const qty = Math.max(1, Number(root.querySelector('[data-el="qty"]')?.value || 1) || 1);
        if (group.key === "free_manual") {
          const label = String(root.querySelector('[data-el="label"]')?.value || "").trim();
        ...
        return {
          ...
          unit_price: amount,
          price_rule_id: rid,
          item_type: String(picked.item_type || "").trim(),
  ...
  const updateTotals_ = () => {
    ...
    const addLine_ = (label, amount, quantity = 1, priceRuleId = "", options = {}) => {
      ...
      if (!lb) return;
      const unit = q > 0 ? Math.max(0, a / q) : a;
      const itemType = String(options?.item_type || "").trim();
      const note = String(options?.note || "").trim();
      const key = buildBillingLineGroupKey_({ item_type: itemType, price_rule_id: priceRuleId, label: lb, unit_price: unit, note });
      const cur = lineMap.get(key) || { label: lb, qty: 0, amount: 0, display_order: resolveDisplayOrder_(priceRuleId) };
      cur.qty += q;
      cur.amount += a;
      const order = resolveDisplayOrder_(priceRuleId);
    ...
    list.forEach((v) => {
      ...
      const subtotal = adjustmentMode
          ? (base + parking + travel + transport + seasonal + extraTotal + (adjustmentOvertimeQty * adjustmentOvertimeAmount) + adjustmentReimbursementAmount)
          : cancellationMode
          ? Math.max(0, Number(host.querySelector(`[name="cancellation_fee_final_amount_${vid}"]`)?.value || 0) || 0)
          : (base + parking + travel + transport + seasonal
          + extraTotal
          + (legacyHeadcountQty * legacyHeadcountAmount)
          + legacyTravelAmount
          + (legacyToppingQty * legacyToppingAmount));
      ...
      if (cancellationMode) {
        const rateType = String(host.querySelector(`[name="cancellation_fee_rate_type_${vid}"]`)?.value || "").trim();
        const label = String(host.querySelector(`[name="cancellation_fee_label_${vid}"]`)?.value || "").trim() || "キャンセル料金";
        addLine_(label, subtotal, 1, String(host.querySelector(`[name="cancellation_fee_price_rule_id_${vid}"]`)?.value || "manual:cancellation_fee"), {
          include_zero: false,
          item_type: "cancellation_fee",
        });
        applyBillingLineModeUi_(vid);
        return;
      }
      ...
      const transportRuleId = parseOptionRuleIdLocal_(host.querySelector(`[name="transport_option_${vid}"]`)?.value || "");
      ...
      addLine_(productLabel, base, 1, basePriceRuleId, { include_zero: hasBaseSelection, item_type: "visit_base" });
      addLine_("駐車料金", parking, 1, parkingRuleId, { item_type: "parking_fee" });
      addLine_("出張料金", travel, 1, travelRuleId, { item_type: "travel_fee" });
      addLine_("交通費", transport, 1, transportRuleId, { item_type: "travel_fee" });
      addLine_("繁忙期加算", seasonal, 1, seasonalRuleId, { item_type: "seasonal_fee" });
      extraLines.forEach((line) => {
        addLine_(line.label, line.quantity * line.unit_price, line.quantity, line.price_rule_id, { item_type: line.item_type, note: line.note });
      });
      if (adjustmentMode && adjustmentOvertimeLabel && adjustmentOvertimeAmount > 0) {
        addLine_(adjustmentOvertimeLabel, adjustmentOvertimeQty * adjustmentOvertimeAmount, adjustmentOvertimeQty, adjustmentOvertimeRuleId, { item_type: "overtime_fee" });
      }
      if (adjustmentMode && adjustmentReimbursementLabel && adjustmentReimbursementAmount > 0) {
        addLine_(adjustmentReimbursementLabel, adjustmentReimbursementAmount, 1, adjustmentReimbursementRuleId, { item_type: "reimbursement" });
      }
      if (legacyMode && legacyHeadcountLabel && legacyHeadcountAmount > 0) {
        addLine_(legacyHeadcountLabel, legacyHeadcountQty * legacyHeadcountAmount, legacyHeadcountQty, legacyHeadcountRuleId, { item_type: "visit_addon" });
      }
      if (legacyMode && legacyTravelLabel && legacyTravelAmount > 0) {
        addLine_(legacyTravelLabel, legacyTravelAmount, 1, legacyTravelRuleId, { item_type: "travel_fee" });
      }
      if (legacyMode && legacyToppingLabel && legacyToppingAmount > 0) {
        addLine_(legacyToppingLabel, legacyToppingQty * legacyToppingAmount, legacyToppingQty, legacyToppingRuleId, { item_type: "visit_addon" });
      }
    });
    ...
    if (keyPickup > 0) {
      ...
      addLine_("鍵預かり料金", keyPickup * keyPickupQty, keyPickupQty, rid, { item_type: "key_pickup_fee" });
    }
    if (keyReturn > 0) {
      ...
      addLine_("鍵返却料金", keyReturn * keyReturnQty, keyReturnQty, rid, { item_type: "key_return_fee" });
    }
  ...
  const refreshDisplay_ = () => {
    list.forEach((v) => {
      ...
      setText_(`transport_amount_${vid}`, transport > 0 ? `${formatMoney_(transport)}円` : "未選択");
      setText_(`seasonal_amount_${vid}`, seasonal > 0 ? `${formatMoney_(seasonal)}円` : "未選択");
      setText_(`visit_option_label_${vid}`, countVisitOptionLinesForVisit_(vid) ? `${countVisitOptionLinesForVisit_(vid)}件` : "未設定");
      setText_(`extra_label_${vid}`, countManualExtraLinesForVisit_(vid) ? `${countManualExtraLinesForVisit_(vid)}件` : "未設定");
  ...
  host.querySelectorAll("[data-edit]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      if (type === "billing_line_mode" && vid) {
        const current = getBillingLineMode_(vid);
        if (current === "cancellation_fee") {
          setValue_(`billing_line_mode_${vid}`, "normal");
          setValue_(`cancellation_fee_rate_type_${vid}`, "");
          setValue_(`cancellation_fee_source_amount_${vid}`, "0");
          setValue_(`cancellation_fee_final_amount_${vid}`, "0");
          setValue_(`cancellation_fee_manual_reason_${vid}`, "");
          refreshDisplay_();
          return;
        }
        const picked = await pickCancellationFeeMode_(vid);
        if (!picked) return;
        setValue_(`billing_line_mode_${vid}`, "cancellation_fee");
        setValue_(`cancellation_fee_rate_type_${vid}`, picked.rate_type);
        setValue_(`cancellation_fee_source_amount_${vid}`, picked.source_amount);
        setValue_(`cancellation_fee_final_amount_${vid}`, picked.final_amount);
        setValue_(`cancellation_fee_manual_reason_${vid}`, picked.manual_reason);
        refreshDisplay_();
        return;
      }
      if (vid && normalFeeEditTypes_.has(type) && getBillingLineMode_(vid) === "cancellation_fee") return;
      if (type === "base" && vid) {
      ...
      if (type === "transport" && vid) {
        const picked = await pickOptionByModal_("交通費を選択", transportOptions);
        if (picked == null) return;
        setValue_(`transport_option_${vid}`, picked);
        const amount = parseOptionAmount_(picked);
        setText_(`transport_amount_${vid}`, amount > 0 ? `${formatMoney_(amount)}円` : "未選択");
        updateTotals_();
        return;
      }
      ...
      if (type === "visit_option" && vid) {
        const chosen = await pickVisitOptionLineByModal_();
        if (!chosen) return;
        addExtraLineRow_(vid, chosen);
        refreshDisplay_();
        return;
      ...
      if (type === "invoice_extra") {
        const chosen = await pickExtraLineByModal_({ scope: "invoice" });
        if (!chosen) return;
        addExtraLineRow_("__invoice__", chosen);
        refreshDisplay_();
        return;
      }
      if (type === "adjustment_item" && vid) {
        const chosen = await pickAdjustmentLineByModal_();
        if (!chosen) return;
        addExtraLineRow_(vid, chosen);
        refreshDisplay_();
        return;
      ...
      if (type === "discount") {
        ...
        const out = await inputDiscountByModal_({
          price_rule_id: String(host.querySelector('[name="discount_price_rule_id"]')?.value || "").trim(),
          label: String(host.querySelector('[name="discount_label"]')?.value || "割引").trim() || "割引",
          amount: curAmount,
        }, discountOptions);
        if (!out) return;
        const amount = Math.max(0, Number(out.amount || 0) || 0);
        setValue_("discount_label", String(out.label || "割引").trim() || "割引");
...
function buildBillingBatchPreviewHtml_(selected, payload, feeDefaults) {
  ...
  const cancellationModeByVisit = new Map((Array.isArray(p?.billing_line_modes) ? p.billing_line_modes : [])
    .filter((x) => String(x?.mode || "").trim() === "cancellation_fee")
    .map((x) => [String(x?.visit_id || "").trim(), x]));
  ...
  const addLine_ = (name, unitPrice, quantity, priceRuleId = "", options = {}) => {
    ...
    const key = buildBillingLineGroupKey_({
  ...
  list.forEach((v) => {
    const vid = String(v?.visit_id || "").trim();
    const cancellationMode = cancellationModeByVisit.get(vid) || null;
    if (cancellationMode) {
      const fee = cancellationMode.cancellation_fee || {};
      const amount = Math.max(0, Number(fee.final_amount || 0) || 0);
      addLine_(String(fee.label || "キャンセル料金").trim() || "キャンセル料金", amount, 1, String(fee.price_rule_id || "manual:cancellation_fee"), { item_type: "cancellation_fee" });
      return;
    }
    ...
    const transport = Math.max(0, Number(ov?.transport_fee_amount || 0) || 0);
    ...
    addLine_(baseLabel, base, 1, String(ov?.price_rule_id || ""), {
      ...
      item_type: "visit_base",
    });
    addLine_("駐車料金", parking, 1, String(ov?.parking_price_rule_id || ""), { item_type: "parking_fee" });
    addLine_("出張料金", travel, 1, String(ov?.travel_price_rule_id || ""), { item_type: "travel_fee" });
    addLine_("交通費", transport, 1, String(ov?.transport_price_rule_id || ""), { item_type: "travel_fee" });
    addLine_("繁忙期加算", seasonal, 1, String(ov?.seasonal_price_rule_id || ""), { item_type: "seasonal_fee" });
    ...
    extras.forEach((x) => {
      ...
      addLine_(label, up, qty, String(x?.price_rule_id || ""), { item_type: x?.item_type, note: x?.note });
    });
  });
  extraLines.forEach((x) => {
    const vid = String(x?.visit_id || "").trim();
  ...
  addLine_("鍵預かり料金", keyPickup, keyPickupQty, String(p?.key_pickup_price_rule_id || ""), { item_type: "key_pickup_fee" });
  addLine_("鍵返却料金", keyReturn, keyReturnQty, String(p?.key_return_price_rule_id || ""), { item_type: "key_return_fee" });
  const rows = Array.from(lineMap.values())
    .map((x) => {
...
function collectExtraLinesFromFormValues_(formValues, visitIds) {
  ...
  Object.keys(lineKeysByVisit).forEach((vid) => {
    lineKeysByVisit[vid].forEach((key) => {
      ...
      const priceRuleId = String(fv[`extra_line_price_rule_id_${vid}_${key}`] || "").trim();
      const itemType = String(fv[`extra_line_item_type_${vid}_${key}`] || "").trim();
      const note = String(fv[`extra_line_note_${vid}_${key}`] || "").trim();
      if (!label || !(amount > 0)) return;
      rows.push({ visit_id: vid === "__invoice__" ? "" : vid, label, quantity: qty, unit_price: amount, price_rule_id: priceRuleId, item_type: itemType, note });
    });
  });
  return rows;
}

function restoreFormValues_(host, formValues) {
  if (!host || !formValues || typeof formValues !== "object") return;
  const hasOwn_ = (key) => Object.prototype.hasOwnProperty.call(formValues, key);
  host.querySelectorAll("[name]").forEach((el) => {
    const name = String(el?.getAttribute("name") || "").trim();
    if (!name || !hasOwn_(name)) return;
    const value = String(formValues[name] == null ? "" : formValues[name]);
    const type = String(el?.type || "").trim().toLowerCase();
    if (type === "checkbox") {
      el.checked = value === "on" || value === "true" || value === "1" || value === String(el.value || "");
      return;
    }
    if (type === "radio") {
      el.checked = value === String(el.value || "");
      return;
    }
    el.value = value;
  });
...
function buildBillingBatchPayload_(customerId, selected, formValues, options = {}) {
  ...
  const legacyMode = options?.legacy_mode === true || (!adjustmentMode && isLegacyModeForVisits_(list, "2026-05-31"));
  const visitIds = list.map((v) => String(v?.visit_id || "").trim()).filter(Boolean);
  ...
  const cancellationModes = list.map((v) => {
    const vid = String(v?.visit_id || "").trim();
    const mode = String(fv[`billing_line_mode_${vid}`] || "normal").trim() === "cancellation_fee" ? "cancellation_fee" : "normal";
    if (!vid || mode !== "cancellation_fee") return null;
    return {
      visit_id: vid,
      mode,
      cancellation_fee: {
        rate_type: String(fv[`cancellation_fee_rate_type_${vid}`] || "").trim(),
        source_amount: Math.max(0, Number(fv[`cancellation_fee_source_amount_${vid}`] || 0) || 0),
        final_amount: Math.max(0, Number(fv[`cancellation_fee_final_amount_${vid}`] || 0) || 0),
        manual_reason: String(fv[`cancellation_fee_manual_reason_${vid}`] || "").trim(),
        price_rule_id: String(fv[`cancellation_fee_price_rule_id_${vid}`] || "manual:cancellation_fee").trim() || "manual:cancellation_fee",
        label: String(fv[`cancellation_fee_label_${vid}`] || "キャンセル料金").trim() || "キャンセル料金",
      },
    };
  }).filter(Boolean);
  const cancellationVisitIds = new Set(cancellationModes.map((x) => String(x?.visit_id || "").trim()).filter(Boolean));
  const visitOverrides = adjustmentMode ? [] : list.map((v) => {
    const vid = String(v?.visit_id || "").trim();
    const cancellationMode = cancellationVisitIds.has(vid);
    ...
    const transportRaw = String(fv[`transport_option_${vid}`] || "").trim();
    const seasonalRaw = String(fv[`seasonal_option_${vid}`] || "").trim();
    return {
      visit_id: vid,
      price_rule_id: cancellationMode ? "" : priceRuleId,
      base_fee_amount: cancellationMode ? 0 : baseFeeAmount,
      base_fee_label: cancellationMode ? "適用しない" : baseFeeLabel,
      parking_fee_amount: cancellationMode ? 0 : parkingFeeAmount,
      parking_price_rule_id: cancellationMode ? "" : String(fv[`parking_price_rule_id_${vid}`] || "").trim(),
      travel_fee_amount: cancellationMode ? 0 : parseOptionAmount_(travelRaw),
      travel_price_rule_id: cancellationMode ? "" : parseOptionRuleId_(travelRaw),
      transport_fee_amount: cancellationMode ? 0 : parseOptionAmount_(transportRaw),
      transport_price_rule_id: cancellationMode ? "" : parseOptionRuleId_(transportRaw),
      seasonal_fee_amount: cancellationMode ? 0 : parseOptionAmount_(seasonalRaw),
      seasonal_price_rule_id: cancellationMode ? "" : parseOptionRuleId_(seasonalRaw),
    };
  }).filter((x) => x.visit_id);
  const customExtraLines = collectExtraLinesFromFormValues_(fv, visitIds)
    .filter((line) => !cancellationVisitIds.has(String(line?.visit_id || "").trim()));
  const extraLines = list.map((v) => {
    const vid = String(v?.visit_id || "").trim();
    if (cancellationVisitIds.has(vid)) return null;
  ...
  const legacyFixedLines = legacyMode ? list.flatMap((v) => {
    const vid = String(v?.visit_id || "").trim();
    if (cancellationVisitIds.has(vid)) return [];
    const rows = [];
  ...
  const sourceExtraLines = Array.isArray(options?.source_extra_lines)
    ...
    })).filter((x) => x.visit_id && x.label && x.unit_price > 0 && !cancellationVisitIds.has(String(x.visit_id || "").trim()))
    : [];
  ...
  return {
    ...
    billing_line_modes: cancellationModes,
...
function buildSourceBatchReissueSeed_(detail, ids) {
  ...
  const putExtraLineFormValues_ = (vid, line) => {
    ...
    formValues[`extra_line_price_rule_id_${vid}_${key}`] = String(line?.price_rule_id || "").trim();
  ...
  (Array.isArray(d.invoice_items) ? d.invoice_items : []).forEach((item) => {
    ...
    if (itemType === "cancellation_fee") {
      const qty = Math.max(1, Number(source?.quantity || item?.quantity || 1) || 1);
      const lineTotal = Math.max(0, qty * amount);
      formValues[`billing_line_mode_${vid}`] = "cancellation_fee";
      formValues[`cancellation_fee_rate_type_${vid}`] = "manual";
      formValues[`cancellation_fee_source_amount_${vid}`] = String(lineTotal);
      formValues[`cancellation_fee_final_amount_${vid}`] = String(lineTotal);
      formValues[`cancellation_fee_manual_reason_${vid}`] = String(source?.note || item?.note || "既存キャンセル料から復元").trim() || "既存キャンセル料から復元";
      formValues[`cancellation_fee_price_rule_id_${vid}`] = rid || "manual:cancellation_fee";
      formValues[`cancellation_fee_label_${vid}`] = label || "キャンセル料金";
      return;
    }
...
export async function renderVisitsList(appEl, query) {
  ...
  const runCreateInvoiceDraft_ = async (options = {}) => {
    ...
    let sourceExistingFees = {
      ...
      key_pickup_quantity: 0,
      key_pickup_conflict: false,
      key_pickup_price_rule_id: "",
      key_return_fee_amount: 0,
      key_return_quantity: 0,
      key_return_conflict: false,
      key_return_price_rule_id: "",
    };
    let selected = selectedVisits_(visitsAll, ids);
    const selectedIdSet = new Set(selected.map((v) => String(v?.visit_id || "").trim()).filter(Boolean));
    const missingSelectedIds = ids.filter((id) => !selectedIdSet.has(id));
    if (sourceBatchId && missingSelectedIds.length) {
      try {
        ...
        const sourceSeedIds = missingSelectedIds;
        ...
        const sourceVisits = Array.isArray(sourceSeed.visits) ? sourceSeed.visits : [];
        selected = selected.concat(sourceVisits.filter((v) => {
          const vid = String(v?.visit_id || "").trim();
          return !!vid && !selectedIdSet.has(vid);
        }));
      } catch (e) {
    ...
    await runWithBlocking_(
      ...
      async () => {
        ...
        try {
          ...
          customerDefaults = {
            customer_fee_settings: Array.isArray(c?.customer_fee_settings) ? c.customer_fee_settings : [],
            customer_fee_settings_resolved: c?.customer_fee_settings_resolved === true,
          };
        } catch (_) {
          customerDefaults = {};
        }
    ...
    if (sourceBatchId) {
      try {
        ...
        let keyPickupQuantity = 0;
        let keyPickupConflict = false;
        let keyReturnFeeAmount = null;
        let keyReturnQuantity = 0;
        let keyReturnConflict = false;
        ...
        lineItems.forEach((line) => {
          ...
          if (itemType === "key_pickup_fee" || label.includes("鍵預かり")) {
            if (keyPickupFeeAmount == null) keyPickupFeeAmount = unit;
            else if (keyPickupFeeAmount !== unit || (keyPickupPriceRuleId && rid && keyPickupPriceRuleId !== rid)) keyPickupConflict = true;
            keyPickupQuantity += qty;
            if (rid) keyPickupPriceRuleId = rid;
          }
          if (itemType === "key_return_fee" || label.includes("鍵返却")) {
            if (keyReturnFeeAmount == null) keyReturnFeeAmount = unit;
            else if (keyReturnFeeAmount !== unit || (keyReturnPriceRuleId && rid && keyReturnPriceRuleId !== rid)) keyReturnConflict = true;
            keyReturnQuantity += qty;
        ...
        sourceExistingFees = {
          ...
          key_pickup_quantity: Math.max(0, Number(keyPickupQuantity || 0) || 0),
          key_pickup_conflict: keyPickupConflict,
          ...
          key_return_quantity: Math.max(0, Number(keyReturnQuantity || 0) || 0),
          key_return_conflict: keyReturnConflict,
        ...
        if (shouldPresetInactiveCancellationRows) {
          selected.forEach((v) => {
            ...
            formValues[`billing_line_mode_${vid}`] = "cancellation_fee";
            formValues[`cancellation_fee_rate_type_${vid}`] = "manual";
            formValues[`cancellation_fee_source_amount_${vid}`] = String(cancelFee);
            formValues[`cancellation_fee_final_amount_${vid}`] = String(cancelFee);
            formValues[`cancellation_fee_manual_reason_${vid}`] = "既存キャンセル料から復元";
            formValues[`cancellation_fee_price_rule_id_${vid}`] = "manual:cancellation_fee";
            formValues[`cancellation_fee_label_${vid}`] = "キャンセル料金";
          });
        }
      } catch (e) {
        toast({ title: "既存請求の取得失敗", message: e?.message || String(e) });
    ...
    while (true) {
      ...
      formValues = await showFormModal({
        ...
        onOpen: (host) => {
          const wired = wireBillingBatchFormInteractions_(host, selected, feeDefaults, {
            legacy_mode: legacyMode,
            adjustment_mode: adjustmentMode,
            customer_defaults: customerDefaults,
          });
          if (formValues && typeof formValues === "object") {
            ...
            restoreFormValues_(host, formValues);
      ...
      if (!adjustmentMode && sourceReissueSeed.visits.length) {
        const sourcePayload = buildBillingBatchPayload_(customerIds[0], sourceReissueSeed.visits, sourceReissueSeed.form_values, {
          ...
          legacy_mode: isLegacyModeForVisits_(sourceReissueSeed.visits, "2026-05-31"),
          fee_defaults: feeDefaults,
          source_extra_lines: sourceReissueSeed.extra_lines,
        });
        const mergeInvoiceLevelFee_ = (sourceUnit, sourceQty, sourceRuleId, sourceConflict, nextUnit, nextQty, nextRuleId, label) => {
          const aUnit = Math.max(0, Number(sourceUnit || 0) || 0);
          const aQty = Math.max(0, Number(sourceQty || 0) || 0);
          const aRid = String(sourceRuleId || "").trim();
          const bUnit = Math.max(0, Number(nextUnit || 0) || 0);
          const bQty = bUnit > 0 ? Math.max(0, Number(nextQty || 0) || 0) : 0;
          const bRid = String(nextRuleId || "").trim();
          if (sourceConflict) {
            return { error: `${label}が既存請求書内で複数種類に分かれているため統合できません。追加請求として別請求にするか、既存請求書の構成を整理してください。` };
          }
          if (aUnit > 0 && bUnit > 0 && (aUnit !== bUnit || (aRid && bRid && aRid !== bRid))) {
            return { error: `${label}が既存請求書と追加分で異なるため統合できません。同じ鍵料金に揃えるか、追加請求として別請求にしてください。` };
          }
          return { amount: aUnit || bUnit, quantity: aQty + bQty, price_rule_id: bRid || aRid };
        };
        const mergedKeyPickup = mergeInvoiceLevelFee_(
          sourceExistingFees.key_pickup_fee_amount,
          sourceExistingFees.key_pickup_quantity,
          sourceExistingFees.key_pickup_price_rule_id,
          sourceExistingFees.key_pickup_conflict,
          payload.key_pickup_fee_amount,
          payload.key_pickup_quantity,
          payload.key_pickup_price_rule_id,
          "鍵預かり料金"
        );
        const mergedKeyReturn = mergeInvoiceLevelFee_(
          sourceExistingFees.key_return_fee_amount,
          sourceExistingFees.key_return_quantity,
          sourceExistingFees.key_return_price_rule_id,
          sourceExistingFees.key_return_conflict,
          payload.key_return_fee_amount,
          payload.key_return_quantity,
          payload.key_return_price_rule_id,
          "鍵返却料金"
        );
        if (mergedKeyPickup.error || mergedKeyReturn.error) {
          toast({ title: "既存請求書に統合できません", message: mergedKeyPickup.error || mergedKeyReturn.error });
          continue;
        }
        payload = Object.assign({}, payload, {
          ...
          billing_line_modes: (Array.isArray(sourcePayload.billing_line_modes) ? sourcePayload.billing_line_modes : [])
            .concat(Array.isArray(payload.billing_line_modes) ? payload.billing_line_modes : []),
          key_pickup_fee_amount: mergedKeyPickup.amount,
          key_pickup_quantity: mergedKeyPickup.quantity,
          key_return_fee_amount: mergedKeyReturn.amount,
          key_return_quantity: mergedKeyReturn.quantity,
          key_pickup_price_rule_id: mergedKeyPickup.price_rule_id,
          key_return_price_rule_id: mergedKeyReturn.price_rule_id,
  ...
  const runAttachVisitsToInvoice_ = async () => {
    ...
    try {
      ...
      candidates = rows.filter((row) => {
        ...
        if (!(status === "billed" || status === "draft")) return false;
        const batchType = normalizeBillingBatchTypeKey_(row?.batch_type || "invoice");
        return batchType === "invoice" || batchType === "invoice_with_cancellation" || batchType === "cancellation_only";
      });
    } catch (e) {
