...
export async function renderSettings(app, query) {
  ...
  render(app, `
    <section class="section">
      ...
      ${isAdmin ? `<div class="card" style="margin-top:12px;">
        <div class="p"><strong>請求バッチ</strong></div>
        <div class="p">Square連携用の請求バッチを確認します。</div>
        <div class="settings-actions" style="display:flex; gap:8px; margin-top:14px; flex-wrap:wrap;">
  ...
  function renderInvoiceList_() {
    ...
    if (!host) return;
    if (!_invoices.length) {
      host.innerHTML = `<p class="p">請求バッチがありません。</p>`;
      return;
    }
  ...
  async function loadInvoices_(opts = {}) {
    ...
    if (host && opts.silent !== true) host.innerHTML = `<p class="p">請求バッチを読み込み中…</p>`;
    try {
      const runner = async () => {
        const idToken = getIdToken();
        const res = await callGas({ action: "listBillingBatches" }, idToken);
        const results = Array.isArray(res?.results) ? res.results : (Array.isArray(res) ? res : []);
        return results;
      };
        ...
        : await runWithBlocking_(
            {
              ...
              bodyHtml: "Square連携用の請求バッチ一覧を取得しています。",
              busyText: "読み込み中..."
            },
            runner
          );
      ...
      if (host) host.innerHTML = `<div class="card"><div class="p">請求バッチ一覧の取得に失敗しました。</div></div>`;
    }
  }
  ...
  qs("#btnSaveBillingRule")?.addEventListener("click", async () => {
    ...
    if (!isVisitBase && !isSeasonal && payload.variations.some((x) => !String(x.variant_name || "").trim())) {
  ...
  qs("#invoiceList")?.addEventListener("click", async (ev) => {
    ...
    if (host) host.innerHTML = `<p class="p">請求バッチ詳細を読み込み中…</p>`;
    try {
      const detail = await runWithBlocking_(
        {
          title: "請求バッチ詳細を読み込んでいます",
          bodyHtml: "Square連携用の請求バッチ詳細を取得しています。",
          busyText: "読み込み中..."
        },
        async () => {
          const idToken = getIdToken();
          return await callGas({ action: "getBillingBatchDetail", batch_id: invoiceId }, idToken);
        }
      );
      if (host) host.innerHTML = invoiceDetailHtml_(detail);
    } catch (e) {
      if (host) host.innerHTML = `<div class="card"><div class="p">請求バッチ詳細の取得に失敗しました。</div></div>`;
