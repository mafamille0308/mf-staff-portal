...
import { escapeHtml, render, showFormModal, showModal, toast } from "../ui.js";
...
const KEY_SUMMARY_RATE_PERCENT_ = "mf:summary:selected_rate_percent:v1";
...
function summaryBaseAmount_(summary) {
  const paidBase = Number(summary?.base_amount_reference || 0) || 0;
  const uncollectedBase = Number(summary?.uncollected_base_amount_reference || 0) || 0;
  return Math.max(0, paidBase + uncollectedBase);
}

function rewardByRate_(baseAmount, ratePercent) {
  const base = Math.max(0, Number(baseAmount || 0) || 0);
  const rate = Math.max(0, Number(ratePercent || 0) || 0);
  return Math.round(base * (rate / 100));
}

function loadSelectedRatePercent_() {
  try {
    const n = Number(localStorage.getItem(KEY_SUMMARY_RATE_PERCENT_) || "");
...
function saveSelectedRatePercent_(ratePercent) {
  try {
    localStorage.setItem(KEY_SUMMARY_RATE_PERCENT_, String(Math.max(0, Number(ratePercent || 0) || 0)));
  } catch (_) {}
}

function effectiveRatePercent_(summary, selectedRatePercent) {
  const selected = Number(selectedRatePercent);
  if (Number.isFinite(selected) && selected >= 0) return selected;
  return Number(summary?.applied_rate_percent || 0) || 0;
}

function rateOptionsHtml_(summary, selectedRatePercent) {
  const values = new Set([50, 55, 60, 65, 70, 75, 80, 100]);
  normalizeTiers_(summary?.tiers).forEach((t) => values.add(Math.round(Number(t?.rate_percent || 0) || 0)));
  const applied = Math.round(Number(summary?.applied_rate_percent || 0) || 0);
  if (applied > 0) values.add(applied);
  const selected = Math.round(Number(selectedRatePercent || applied || 0) || 0);
  if (selected > 0) values.add(selected);
  return Array.from(values)
    .filter((v) => Number.isFinite(v) && v >= 0)
    .sort((a, b) => a - b)
    .map((v) => `<option value="${escapeHtml(String(v))}"${v === selected ? " selected" : ""}>${escapeHtml(String(v))}%</option>`)
    .join("");
}

function renderMonthPickerBody_(ym) {
  const safeYm = parseYm_(ym) ? ym : currentYearMonthJst_();
  return `
    <form class="summary-month-form">
      <label class="summary-month-form-label">
        <span>対象月</span>
        <input class="summary-month-form-input" type="month" name="year_month" value="${escapeHtml(String(safeYm))}" required>
      </label>
    </form>
  `;
}
...
function renderSummaryDetailModalBody_(detail, ratePercent) {
  const rows = Array.isArray(detail?.rows) ? detail.rows : [];
  const rate = Math.max(0, Number(ratePercent || detail?.applied_rate_percent || 0) || 0);
  const tableRows = rows.map((row, idx) => {
    const markers = row?.markers || {};
    const baseAmount = Number(row?.base_amount_reference || 0) || 0;
    const estimatedReward = rewardByRate_(baseAmount, rate);
    const notes = [
    ...
    return `
      <tr>
        ...
        <td class="text-right nowrap">${escapeHtml(formatMoney_(baseAmount))}円</td>
        <td class="text-right nowrap">${escapeHtml(formatMoney_(estimatedReward))}円</td>
  ...
  return `
    <div class="summary-detail-modal">
      <div class="summary-detail-note-box">
        報酬額は選択報酬率 ${escapeHtml(String(Math.round(rate)))}% で再計算しています。請求書全体割引の按分、最低保証、個別補填はここでは反映していません。
      </div>
...
function heroHtml_(summary, opts = {}) {
  ...
  const selectedRate = effectiveRatePercent_(summary, opts.selectedRatePercent);
  const paidBase = Number(summary.base_amount_reference || 0) || 0;
  const uncollectedBase = Number(summary.uncollected_base_amount_reference || 0) || 0;
  const totalBase = summaryBaseAmount_(summary);
  const paidReward = rewardByRate_(paidBase, selectedRate);
  const uncollectedReward = rewardByRate_(uncollectedBase, selectedRate);
  const totalReward = rewardByRate_(totalBase, selectedRate);
  ...
  const { list: tiers } = resolveTierProgress_(count, summary?.tiers);
  const motivation = `現在${formatMoney_(count)}件稼働`;
  ...
  return `
    <div class="card summary-hero">
      <div class="summary-hero-top">
        <button type="button" id="summaryMonthPicker" class="summary-month-picker" title="対象月を変更" aria-label="対象月を変更">
      ...
      <div class="summary-hero-main">
        <div class="summary-rate-wrap">
          <div class="summary-rate-donut" style="--summary-rate-gradient:${escapeHtml(donutGradient)};">
            ...
            <div class="summary-rate-center">
              <label class="summary-rate-picker">
                <span class="summary-rate-label">報酬率</span>
                <select id="summaryRateSelect" class="summary-rate-select" aria-label="報酬率">
                  ${rateOptionsHtml_(summary, selectedRate)}
                </select>
              </label>
            </div>
          </div>
        ...
        <div class="summary-hero-metrics">
          ...
          <div class="summary-hero-metric">
            <span class="summary-hero-metric-label">回収済み報酬額<span class="summary-hero-metric-note">売上ベース ${escapeHtml(formatMoney_(paidBase))}円 / ${escapeHtml(String(Math.round(selectedRate)))}%計算</span></span>
          ...
          <div class="summary-hero-metric">
            <span class="summary-hero-metric-label">未回収稼働分 報酬見込<span class="summary-hero-metric-note">売上ベース ${escapeHtml(formatMoney_(uncollectedBase))}円 / ${escapeHtml(String(Math.round(selectedRate)))}%計算${unpricedNote ? ` / ${escapeHtml(unpricedNote)}` : ""}</span></span>
          ...
          <div class="summary-hero-metric">
            <span class="summary-hero-metric-label">当月概算報酬額<span class="summary-hero-metric-note">売上ベース合計 ${escapeHtml(formatMoney_(totalBase))}円 / ${escapeHtml(String(Math.round(selectedRate)))}%計算</span></span>
...
function toMonthlyPoint_(summary, ym, ratePercent = null) {
  const parsed = parseYm_(ym);
  const month = parsed ? parsed.month : 0;
  const year = parsed ? parsed.year : 0;
  const count = Number(summary?.completed_active_count ?? summary?.done_paid_active_count ?? 0) || 0;
  const baseAmount = summaryBaseAmount_(summary);
  const reward = ratePercent == null ? totalEstimatedReward_(summary) : rewardByRate_(baseAmount, ratePercent);
...
function aggregateYearly_(monthlyPoints) {
  ...
  monthlyPoints.forEach((p) => {
    ...
    const cur = map.get(y) || { key: y, label: String(parsed.year), subLabel: "", periodLabel: `${parsed.year}年`, count: 0, baseAmount: 0, reward: 0, latestYm: "" };
    cur.count += Number(p.count || 0);
    cur.baseAmount += Number(p.baseAmount || 0);
...
export async function renderSummaryPage(appEl) {
  ...
  const initialSummary = byYm[anchorYm] || null;
  let selectedRatePercent = loadSelectedRatePercent_();
  if (selectedRatePercent == null) selectedRatePercent = effectiveRatePercent_(initialSummary, null);
  const monthlyPoints = monthKeys.map((ym) => toMonthlyPoint_(byYm[ym], ym, selectedRatePercent));
  ...
  const rebuildMonthlySeries_ = () => {
    state.monthlyPoints = state.monthlyKeys.map((ym) => toMonthlyPoint_(byYm[ym], ym, selectedRatePercent));
  ...
  const openSummaryDetail_ = async (summary) => {
    ...
    try {
      ...
      await showModal({
        title: "当月概算報酬額の内訳",
        bodyHtml: renderSummaryDetailModalBody_(detail, selectedRatePercent),
        okText: "閉じる",
        cancelText: "",
      });
    } catch (e) {
  ...
  const selectMonth_ = async (ym) => {
    const targetYm = String(ym || "").trim();
    if (!parseYm_(targetYm)) return;
    if (!Object.prototype.hasOwnProperty.call(byYm, targetYm)) {
      const bundle = await fetchMonthlySummaries_(idToken, [targetYm], staffId);
      Object.assign(byYm, bundle.byYm || {});
      if (Number(bundle.errorCount || 0) > 0) {
        toast({ title: "取得失敗", message: "選択月の集計取得に失敗したため、0件で表示しています。" });
      }
    }
    if (!state.monthlyKeys.includes(targetYm)) {
      state.monthlyKeys = [...state.monthlyKeys, targetYm].sort((a, b) => a.localeCompare(b));
    }
    state.mode = "monthly";
    state.selectedKey = targetYm;
    rebuildMonthlySeries_();
    switchMode_("monthly");
    requestAnimationFrame(() => {
      const offset = barOffsetLeft_(viewportEl, barsEl, targetYm);
      if (offset == null) return;
      viewportEl.scrollLeft += offset - Math.max(0, (viewportEl.clientWidth / 2) - 20);
    });
  };
  ...
  const updateDetail_ = () => {
    ...
    const monthPickerEl = heroHost.querySelector("#summaryMonthPicker");
    if (monthPickerEl && !monthPickerEl.dataset.bound) {
      monthPickerEl.dataset.bound = "1";
      monthPickerEl.addEventListener("click", async () => {
        const currentYm = String(summary?.year_month || state.selectedKey || currentYearMonthJst_()).trim();
        const form = await showFormModal({
          title: "対象月を変更",
          bodyHtml: renderMonthPickerBody_(currentYm),
          okText: "表示",
          cancelText: "キャンセル",
          formSelector: ".summary-month-form",
        });
        const nextYm = String(form?.year_month || "").trim();
        if (nextYm) void selectMonth_(nextYm);
      });
    }
    const rateSelectEl = heroHost.querySelector("#summaryRateSelect");
    if (rateSelectEl && !rateSelectEl.dataset.bound) {
      rateSelectEl.dataset.bound = "1";
      rateSelectEl.addEventListener("change", () => {
        selectedRatePercent = Math.max(0, Number(rateSelectEl.value || 0) || 0);
        saveSelectedRatePercent_(selectedRatePercent);
        rebuildMonthlySeries_();
        renderBars_();
        updateDetail_();
      });
