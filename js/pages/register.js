...
import { searchStaffsPolicy_, listCustomerAssignmentsPolicy_, bulkRegisterVisitsPolicy_ } from "./register_policy.js";
...
function uiIconSvg_(name) {
  ...
  if (name === "trash") {
    return `<svg ${common}><path d="M3 6h18"></path><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"></path><path d="M10 11v6"></path><path d="M14 11v6"></path></svg>`;
  }
  if (name === "calendar") {
    return `<svg ${common}><rect x="3" y="4" width="18" height="18" rx="2"></rect><path d="M16 2v4"></path><path d="M8 2v4"></path><path d="M3 10h18"></path></svg>`;
  }
  return "";
}
...
function buildCourseOptionsFromVisitBaseRules_() {
  const map = new Map();
  (Array.isArray(_visitBaseRulesCache) ? _visitBaseRulesCache : []).forEach((x) => {
    const mins = Number(x?.duration_minutes || 0) || 0;
    if (!(mins > 0)) return;
    const course = String(x?.course || `${mins}min`).trim();
    if (!course) return;
    if (!map.has(course)) map.set(course, { course, minutes: mins });
  });
  const list = Array.from(map.values()).sort((a, b) => a.minutes - b.minutes);
  return list.length ? list : fallbackCourseOptions_();
}
 ...
 * - 変換できない場合は空文字
 * - slot_minutes 等の丸めは UIではやらない（APIが最終確定）
 */
...
function newRequestId_() {
  // RequestLogs / 冪等設計に乗せる（再送も同一request_idを使う）
...
async function ensureVisitBaseRules_() {
  ...
  try {
    ...
    const resp = await listBillingPriceRulesPolicy_(idToken, true);
    ...
    _courseOptionsCache = buildCourseOptionsFromVisitBaseRules_();
  } catch (_) {
...
export function renderRegisterTab(app) {
  ...
  ensureVisitBaseRules_().then(() => { try { populateCommonProductOptions_(); populateCommonVariantOptions_(); refreshUI_(); } catch (e) {} });
  ...
  async function loadAssignStaffOptions_() {
    ...
    try {
      const [staffRes, assignRes] = await Promise.all([
        searchStaffsPolicy_(getIdToken()),
        listCustomerAssignmentsPolicy_(getIdToken(), {
  ...
  function renderWarnings_(warnings = []) {
    ...
    warningsEl.innerHTML = `<div class="card"><p class="p text-danger"><b>注意</b>（登録前に確認してください）</p>${html}</div>`;
  ...
  function renderEditor_(draft) {
    ...
    const cards = visits.map((v, idx) => {
      ...
      const productName = String(v.product_name || "").trim();
        ...
        <div class="preview-card" data-idx="${idx}" style="padding:12px; margin-bottom:12px; border:1px solid #ddd; border-radius:8px;">
          ...
          <div style="margin-bottom:12px; padding-bottom:12px; border-bottom:1px solid #eee;">
            <div style="display:flex; justify-content:space-between; align-items:center; gap:8px; margin-bottom:8px;">
              <div style="font-size:15px; font-weight:600; flex:1; min-width:0;">
                <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                  <div style="overflow:hidden; text-overflow:ellipsis; white-space:nowrap; max-width:45vw;">
                    <span style="display:inline-flex; align-items:center; gap:4px;">${uiIconSvg_("calendar")}<span>#${escapeHtml(rowNum)}</span></span>
                  </div>
                  <input type="date"
                    inputmode="numeric"
                    class="input mono"
              ...
              <div style="display:flex; gap:6px; flex-shrink:0;">
                <button class="btn btn-sm" type="button" data-action="dup" title="複製" aria-label="複製" style="padding:4px 8px; min-width:auto; display:inline-flex; align-items:center; justify-content:center;">${uiIconSvg_("copy")}</button>
                <button class="btn btn-sm" type="button" data-action="del" title="削除" aria-label="削除" style="padding:4px 8px; min-width:auto; color:#d32f2f; display:inline-flex; align-items:center; justify-content:center;">${uiIconSvg_("trash")}</button>
              </div>
            </div>
          ...
          <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:10px; margin-bottom:10px;">
            <div style="min-width:0;">
              ...
              <input type="time" class="input mono" data-field="start_time" value="${escapeHtml(st || "09:00")}" style="display:block; width:100%; max-width:100%; min-width:0; font-size:14px; -webkit-appearance:none; appearance:none;" />
            </div>
            <div style="min-width:0;">
              <label class="label-sm" style="display:block; margin-bottom:4px; font-weight:600; color:#555; font-size:12px;">終了</label>
              <input class="input mono" value="${escapeHtml(endHm)}" disabled style="display:block; width:100%; max-width:100%; min-width:0; font-size:14px;" />
            </div>
          </div>
  ...
  if (commitBtn) commitBtn.addEventListener("click", async () => {
    ...
    // commit payload：end_time は送らない（APIで start_time と料金ルールから再計算）
    const visitsForCommit = visits.map((v) => {
      const nv = { ...v };
      // UIでは end_time を扱わない。存在しても送らない。
      try { delete nv.end_time; } catch (e) {}
      // API側で保持しない互換項目は送らない。
      try { delete nv.course; } catch (e) {}
      try { delete nv.visit_index; } catch (e) {}
      try { delete nv.index; } catch (e) {}
    ...
    try {
      ...
      const resp = await bulkRegisterVisitsPolicy_(idToken, {
