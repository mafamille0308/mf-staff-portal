...
export function renderRegisterTab(app) {
      ...
      return `
        <div class="preview-card" data-idx="${idx}" style="padding:12px; margin-bottom:12px; border:1px solid #ddd; border-radius:8px;">
          ...
          <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(120px, 1fr)); gap:10px; margin-bottom:10px;">
            ...
            <div>
              <label class="label-sm" style="display:block; margin-bottom:4px; font-weight:600; color:#555; font-size:12px;">🏷️ 商品</label>
              <input class="input" value="${escapeHtml(productName || "-")}" disabled style="font-size:14px;" />
            </div>
          </div>
  ...
  if (commitBtn) commitBtn.addEventListener("click", async () => {
    ...
    // commit payload：end_time は送らない（APIで start_time + course から再計算）
