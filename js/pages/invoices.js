...
import {
  ...
  resyncBillingBatchVisitStatusesPolicy_,
...
import {
  ...
  pickRuleByModal_,
  inputAmountByModal_,
  inputCancellationFeeRateByModal_,
  inputDiscountByModal_,
...
function squareInvoiceStatusLabel_(status) {
  const s = String(status || "").trim().toLowerCase();
  if (s === "draft") return "下書き";
  if (s === "pending" || s === "unpaid") return "未払い";
  if (s === "published" || s === "sent" || s === "scheduled") return "送信済";
  if (s === "partially_paid") return "一部入金済";
  if (s === "paid" || s === "completed") return "入金済";
  if (s === "partially_refunded") return "一部返金済み";
  if (s === "refunded") return "全額返金済み";
  if (s === "canceled" || s === "cancelled" || s === "voided") return "キャンセル済み";
  return s || "-";
}

function squareInvoiceStatusBadgeClass_(status) {
  const s = String(status || "").trim().toLowerCase();
  if (s === "paid" || s === "completed" || s === "refunded") return "badge badge-success";
  if (s === "partially_paid" || s === "published" || s === "sent" || s === "scheduled" || s === "partially_refunded") return "badge badge-warning";
  if (s === "draft" || s === "pending" || s === "unpaid") return "badge badge-info";
  if (s === "canceled" || s === "cancelled" || s === "voided") return "badge badge-danger";
  return "badge badge-info";
}
...
function actionIconSvg_(name) {
  ...
  if (name === "external") {
    return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M15 3h6v6"/><path d="M10 14 21 3"/><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/></svg>';
  }
  if (name === "refresh") {
    return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 12a9 9 0 0 0-15-6.7L3 8"/><path d="M3 3v5h5"/><path d="M3 12a9 9 0 0 0 15 6.7L21 16"/><path d="M16 16h5v5"/></svg>';
  }
  return "";
}
...
function isRefundFollowupResolved_(status) {
  const s = normalizeRefundFollowupStatus_(status);
  return s === "not_required" || s === "refunded_partial" || s === "refunded_full";
}
...
async function resyncBillingBatchVisitStatuses_(batchId) {
  return withIdToken_((token) => resyncBillingBatchVisitStatusesPolicy_(token, batchId));
}
...
function renderBillingListCard_(b) {
  ...
  const refundFollowupStatus = deriveRefundFollowupStatus_(x, {
    has_inactive_linked_visit: hasInactiveLinkedVisit,
  });
  const refundFollowupLabel = refundFollowupLabel_(refundFollowupStatus);
  const refundFollowupResolved = isRefundFollowupResolved_(refundFollowupStatus);
  const rawInvoiceReconcileRequired = x.invoice_reconcile_required === true || String(x.invoice_reconcile_required || "").toLowerCase() === "true";
  const invoiceReconcileRequired = rawInvoiceReconcileRequired && !(refundFollowupResolved && hasInactiveLinkedVisit);
      ...
      <div class="p invoice-batch-body">
        <div class="row row-between" style="gap:8px;">
          ...
          <div class="row" style="gap:6px;">
            ...
            ${invoiceReconcileRequired ? `<span class="badge badge-warning">要調整</span>` : ``}
      ...
      <section class="section">
        ...
        <div class="edit-grid" style="margin-top:10px;">
          <label>
            ...
            <input id="invoiceFilterKeyword" class="input" type="text" value="${escapeHtml(keyword)}" placeholder="顧客名" />
          </label>
          <label>
...
function buildDetailReadonlyHtml_(detail) {
  ...
  const refundFollowupLabel = refundFollowupLabel_(refundFollowupStatus);
  const refundFollowupResolved = isRefundFollowupResolved_(refundFollowupStatus);
  const reconcileRequiredLinks = links.filter((x) => x?.invoice_reconcile_required === true || String(x?.invoice_reconcile_required || "").toLowerCase() === "true");
  const actionableReconcileRequiredLinks = reconcileRequiredLinks.filter((x) => !(refundFollowupResolved && x && x.visit && x.visit.is_active === false));
  const invoiceReconcileRequired = actionableReconcileRequiredLinks.length > 0;
  const reconcileRequiredVisitCount = actionableReconcileRequiredLinks.length;
  ...
  const refundNotice = (refundFollowupStatus === "refunded_partial" || refundFollowupStatus === "refunded_full")
    ...
    : "";
  const reconcileNotice = hasInactiveLinkedVisit && !refundFollowupResolved
    ? [
      inactiveLinkedVisitCount > 0 ? `キャンセル済み予約が${inactiveLinkedVisitCount}件含まれます。` : "",
      "請求内容が正しいかご確認ください。"
    ].filter(Boolean).join("")
    : "";
  const invoiceReconcileNotice = invoiceReconcileRequired
    ? [
      reconcileRequiredVisitCount > 0 ? `訪問商品変更により要調整の予約が${reconcileRequiredVisitCount}件あります。` : "訪問商品変更により要調整の予約があります。",
      "請求明細を確認し、必要に応じてこの請求を削除または編集してください。"
    ].join("")
    : "";
      ...
      <div class="p">
        <div>
          <div>
            ...
            <div class="row row-between" style="margin-top:4px; gap:8px;">
              <div style="opacity:.85;">
                ...
                ${invoiceReconcileRequired ? `<span class="badge badge-warning badge-inline">要調整</span>` : ``}
              ...
              <button class="btn btn-icon-action" type="button" data-action="resync-visit-statuses" data-batch-id="${escapeHtml(String(batch.batch_id || ""))}" title="請求ステータスを再同期" aria-label="請求ステータスを再同期">${actionIconSvg_("refresh")}</button>
            </div>
            <div class="row row-between" style="margin-top:4px; gap:8px;">
              <div style="opacity:.85;">
                Square状態:
                <span class="${escapeHtml(squareInvoiceStatusBadgeClass_(batch.square_invoice_status))}">${escapeHtml(squareInvoiceStatusLabel_(batch.square_invoice_status))}</span>
              </div>
              ${batch.square_invoice_url ? `<button class="btn btn-icon-action" type="button" data-action="open-square-invoice" data-url="${escapeHtml(String(batch.square_invoice_url || ""))}" title="Square請求書を開く" aria-label="Square請求書を開く">${actionIconSvg_("external")}</button>` : ``}
            </div>
            ...
            ${invoiceReconcileNotice ? `<div style="margin-top:8px; color:var(--state-warn-fg);">${escapeHtml(invoiceReconcileNotice)}</div>` : ``}
          ...
          ${canMarkRefundNotRequired || showDeleteButton ? `<div>
      ...
      <div class="p">
        ...
        <div style="display:grid; gap:8px; margin-top:8px;">
          ${links.map((link) => {
            ...
            const rawLinkReconcileRequired = link?.invoice_reconcile_required === true || String(link?.invoice_reconcile_required || "").toLowerCase() === "true";
            const linkReconcileRequired = rawLinkReconcileRequired && !(refundFollowupResolved && !isActive);
            ...
            return `
              <div style="border-top:1px solid rgba(255,255,255,0.08); padding-top:8px;">
                <div>
                  ...
                  ${refundFollowupLabel ? `<span class="${refundFollowupBadgeClass_(refundFollowupStatus)} badge-inline">${escapeHtml(refundFollowupLabel)}</span>` : ``}
                  ${linkReconcileRequired ? `<span class="badge badge-warning badge-inline">要調整</span>` : ``}
...
function buildRuleCatalog_(rules) {
  ...
  list.forEach((r) => {
    ...
    const storeCategory = String(r?.store_category || "").trim();
    const unitLabel = String(r?.unit_label || "").trim();
    const invoiceInclusion = String(r?.invoice_inclusion || "").trim();
    ...
    if (invoiceInclusion) {
      if (!byInclusion[invoiceInclusion]) byInclusion[invoiceInclusion] = [];
      byInclusion[invoiceInclusion].push(row);
    }
  });
  [byType, byInclusion].forEach((bucket) => {
  ...
  return { byId, byType, byInclusion };
}

function categoryKeyToItemType_(categoryKey, fallback = "") {
  const key = String(categoryKey || "").trim();
  if (key === "visit_primary") return "visit_base";
  if (key === "visit_addon") return "visit_addon";
  if (key === "parking_fee") return "parking_fee";
  if (key === "travel_fee" || key === "business_trip_fee") return "travel_fee";
  if (key === "seasonal_fee") return "seasonal_fee";
  if (key === "extension_fee") return "overtime_fee";
  if (key === "cancellation_fee") return "cancellation_fee";
  if (key === "adjustment") return "discount";
  if (key === "manual_item") return "custom";
  return String(fallback || "").trim();
}

function manualLineRuleOptions_(ruleCatalog) {
  ...
  const push_ = (row) => {
    ...
    if (itemType === "discount" || categoryKey === "adjustment") return;
    if (itemType === "visit_addon" || categoryKey === "visit_addon") return;
    seen[rid] = true;
    out.push(row);
  };
  (Array.isArray(catalog.byInclusion?.manual) ? catalog.byInclusion.manual : []).forEach(push_);
  ["merchandise", "reimbursement", "cancellation_fee", "overtime_fee"].forEach((itemType) => {
    (Array.isArray(catalog.byType?.[itemType]) ? catalog.byType[itemType] : []).forEach(push_);
  });
  return out.sort((a, b) => {
    if (a.display_order !== b.display_order) return a.display_order - b.display_order;
    return String(a.price_rule_id || "").localeCompare(String(b.price_rule_id || ""));
...
function visitOptionLineRuleOptions_(ruleCatalog) {
  const catalog = ruleCatalog || {};
  return (Array.isArray(catalog.byType?.visit_addon) ? catalog.byType.visit_addon : [])
    .filter((row) => String(row?.invoice_inclusion || "").trim() === "manual")
    .sort((a, b) => {
      if (a.display_order !== b.display_order) return a.display_order - b.display_order;
      return String(a.price_rule_id || "").localeCompare(String(b.price_rule_id || ""));
    });
...
function ruleCatalogOptionsByInclusion_(ruleCatalog, itemType, inclusion) {
  const t = String(itemType || "").trim();
  const inc = String(inclusion || "").trim();
  return (Array.isArray(ruleCatalog?.byType?.[t]) ? ruleCatalog.byType[t] : [])
    .filter((row) => String(row?.invoice_inclusion || "").trim() === inc);
}

function rowRuleHasInclusion_(row, ruleCatalog, inclusion) {
  const rid = String(row?.price_rule_id || "").trim();
  const rule = ruleCatalog?.byId?.[rid] || null;
  return !!rule && String(rule.invoice_inclusion || "").trim() === String(inclusion || "").trim();
}

function asItemType_(row, ruleCatalog) {
  const rid = String(row?.price_rule_id || "").trim();
  if (rid === "manual:cancellation_fee") return "cancellation_fee";
  const rule = ruleCatalog?.byId?.[rid] || {};
  const direct = String(rule.item_type || "").trim();
  if (direct) return direct;
  const byCategory = categoryKeyToItemType_(rule.category_key);
...
function isEditorCancellationFeeMode_(visit) {
  return String(visit?.billing_line_mode || "").trim() === "cancellation_fee"
    && Math.max(0, Number(visit?.cancellation_fee_amount || 0) || 0) > 0;
}

function computeEditorVisitNormalSourceAmount_(visit) {
  const v = visit || {};
  return Math.max(0,
    Math.max(0, Number(v.base_amount || 0) || 0)
...
function buildEditorCancellationFeeLines_(visits, legacyMode) {
  if (legacyMode) return [];
  return (Array.isArray(visits) ? visits : [])
    .map((v) => {
      if (!isEditorCancellationFeeMode_(v)) return null;
      const visitId = String(v?.visit_id || "").trim();
      if (!visitId) return null;
      const finalAmount = Math.max(0, Number(v?.cancellation_fee_amount || 0) || 0);
      if (!(finalAmount > 0)) return null;
      const sourceAmount = Math.max(0, Number(v?.cancellation_fee_source_amount || 0) || 0)
        || computeEditorVisitNormalSourceAmount_(v)
        || finalAmount;
      return {
        visit_id: visitId,
        rate_type: String(v?.cancellation_fee_rate_type || "manual").trim() || "manual",
        source_amount: sourceAmount,
        final_amount: finalAmount,
        manual_reason: String(v?.cancellation_fee_manual_reason || "").trim(),
        price_rule_id: String(v?.cancellation_fee_rule_id || "").trim() || "manual:cancellation_fee",
        label: String(v?.cancellation_fee_label || "キャンセル料金").trim() || "キャンセル料金",
      };
    })
    .filter(Boolean);
}

async function inputExtraLineByModal_(current = {}, ruleOptions = []) {
  ...
  const currentRuleId = String(current?.rule_id || current?.price_rule_id || "").trim();
  const options = Array.isArray(ruleOptions) ? ruleOptions : [];
  const allowFree = current?.allow_free !== false;
  const hasCurrentRule = !!currentRuleId && currentRuleId !== "manual:custom";
  const defaultCatalogMode = (hasCurrentRule || !label) && options.length > 0;
  const out = await openEditorModal_({
    title: String(current?.modal_title || "").trim() || "明細追加",
    bodyHtml: `
      <div style="display:grid; gap:8px;">
        <label>
          ...
          <select class="input" data-el="mode">
            ...
            ${allowFree ? `<option value="free" ${defaultCatalogMode ? "" : "selected"}>自由入力で追加</option>` : ``}
          </select>
        </label>
        <label data-mode-section="catalog">
          <div class="label-sm">料金マスタ</div>
          <select class="input" data-el="rule">
            <option value="">選択しない</option>
            ${options.map((o) => `<option value="${escapeHtml(String(o.price_rule_id || ""))}" ${String(o.price_rule_id || "") === currentRuleId ? "selected" : ""}>${escapeHtml(String(o.label || o.price_rule_id || ""))}（${escapeHtml(formatMoney_(o.amount || 0))}円）</option>`).join("")}
          </select>
        </label>
    ...
    onOpen: (root) => {
      ...
      const fillCatalogAmount_ = () => {
        ...
        const picked = options.find((x) => String(x?.price_rule_id || "").trim() === rid) || null;
        const amountEl = root.querySelector('[data-el="amount"]');
        if (picked && amountEl && !String(amountEl.value || "").trim()) {
          amountEl.value = String(Math.max(0, Number(picked.amount || 0) || 0));
        }
      };
    ...
    onSubmit: (root) => {
      ...
      if (mode === "catalog") {
        ...
        const catalogAmount = nextAmount > 0 ? nextAmount : Math.max(0, Number(picked.amount || 0) || 0);
        if (!(catalogAmount > 0)) return null;
        return {
          rule_id: rid,
      ...
      return { rule_id: "manual:custom", label: nextLabel, amount: nextAmount, qty: nextQty };
    }
  });
...
function buildEditorModel_(detail, links, ruleCatalog, options = {}) {
  ...
  const customerFeeDefaults = (detail?.customer_fee_defaults && typeof detail.customer_fee_defaults === "object") ? detail.customer_fee_defaults : {};
  const customerFeeSettings = Array.isArray(customerFeeDefaults.customer_fee_settings) ? customerFeeDefaults.customer_fee_settings : [];
  const resolveCustomerFeeDefault_ = (categoryKey, feeRole, availableRules) => {
    const setting = customerFeeSettings.find((row) => (
    ...
    const rule = (Array.isArray(availableRules) ? availableRules : [])
      .find((candidate) => String(candidate?.price_rule_id || "").trim() === rid) || null;
    const amount = String(setting?.value_mode || "").trim() === "fixed_amount"
      ? Math.max(0, Number(setting?.amount_override || 0) || 0)
      : Math.max(0, Number(rule?.amount || 0) || 0);
    return { amount, rule };
  };
  const cancellationFeeBasisByVisitId = {};
  (Array.isArray(detail?.cancellation_fee_lines) ? detail.cancellation_fee_lines : []).forEach((line) => {
    const vid = String(line?.visit_id || "").trim();
    if (vid && !cancellationFeeBasisByVisitId[vid]) cancellationFeeBasisByVisitId[vid] = line;
  });
  const visitBaseAutoOptions = ruleCatalogOptionsByInclusion_(ruleCatalog, "visit_base", "per_visit");
  const parkingAutoOptions = ruleCatalogOptionsByInclusion_(ruleCatalog, "parking_fee", "per_visit");
  const travelAutoOptions = ruleCatalogOptionsByInclusion_(ruleCatalog, "travel_fee", "per_visit");
  ...
  const visitRows = links.map((link) => {
    ...
    const visit = (link?.visit && typeof link.visit === "object") ? link.visit : {};
    const title = String(visit?.title || vid || "").trim();
    const byVisit = sourceRows.filter((x) => String(x?.visit_id || "").trim() === vid);
    const pickAutoType_ = (t) => byVisit.find((x) => asItemType_(x, ruleCatalog) === t && rowRuleHasInclusion_(x, ruleCatalog, "per_visit")) || null;
    const base = pickAutoType_("visit_base");
    const parking = pickAutoType_("parking_fee");
    const travel = pickAutoType_("travel_fee");
    const seasonal = pickAutoType_("seasonal_fee");
    const autoRows = new Set([base, parking, travel, seasonal].filter(Boolean));
    const extras = byVisit.filter((x) => !autoRows.has(x));
    const cancellationFeeRows = extras.filter((x) => asItemType_(x, ruleCatalog) === "cancellation_fee");
    const legacyHeadcountRows = legacyMode ? extras.filter((x) => isLegacyHeadcountLabel_(x?.label)) : [];
    const legacyTravelRows = legacyMode ? extras.filter((x) => isLegacyTravelLabel_(x?.label)) : [];
    const legacyToppingRows = legacyMode ? extras.filter((x) => isLegacyToppingLabel_(x?.label)) : [];
    const cancellationFee = cancellationFeeRows[0] || null;
    const cancellationFeeMode = !!cancellationFee;
    const cancellationFeeBasis = cancellationFeeBasisByVisitId[vid] || {};
    const visitBaseRuleId = String(visit?.price_rule_id || "").trim();
    const visitBaseRule = ruleCatalog?.byId?.[visitBaseRuleId] || null;
    const defaultBaseRule = visitBaseRule && asItemType_({ price_rule_id: visitBaseRuleId }, ruleCatalog) === "visit_base"
      ? visitBaseRule
      : null;
    const parkingDefault = resolveCustomerFeeDefault_("parking_fee", "parking", parkingAutoOptions);
    const defaultParkingAmount = parkingDefault.amount;
    const defaultParkingRule = parkingDefault.rule || parkingAutoOptions[0] || null;
    const travelDefault = resolveCustomerFeeDefault_("business_trip_fee", "business_trip", travelAutoOptions);
    const defaultTravelAmount = travelDefault.amount;
    const defaultTravelRule = travelDefault.rule;
    ...
    const extraLines = extras.filter((x) => {
      ...
      if (legacyMode && (isLegacyHeadcountLabel_(x?.label) || isLegacyTravelLabel_(x?.label) || isLegacyToppingLabel_(x?.label))) return false;
      return true;
    });
    ...
    return {
      ...
      is_active: link?.is_active !== false && visit?.is_active !== false,
      invoice_reconcile_required: link?.invoice_reconcile_required === true || visit?.invoice_reconcile_required === true,
      billing_line_mode: cancellationFeeMode ? "cancellation_fee" : "normal",
      base_rule_id: String(base?.price_rule_id || (cancellationFeeMode ? defaultBaseRule?.price_rule_id : "") || "").trim(),
      base_label: String(base?.label || (ruleCatalog?.byId?.[String(base?.price_rule_id || "").trim()] || {}).label || (cancellationFeeMode ? defaultBaseRule?.label : "") || "").trim(),
      base_amount: Math.max(0, Number(base?.unit_price_snapshot || (cancellationFeeMode ? defaultBaseRule?.amount : 0) || 0) || 0),
      parking_rule_id: String(parking?.price_rule_id || (cancellationFeeMode ? defaultParkingRule?.price_rule_id : "") || ((ruleCatalogOptionsByInclusion_(ruleCatalog, "parking_fee", "per_visit") || [])[0] || {}).price_rule_id || "").trim(),
      parking_amount: Math.max(0, Number(parking?.unit_price_snapshot || (cancellationFeeMode ? defaultParkingAmount : 0) || 0) || 0),
      travel_rule_id: String(travel?.price_rule_id || (cancellationFeeMode ? defaultTravelRule?.price_rule_id : "") || "").trim(),
      travel_amount: Math.max(0, Number(travel?.unit_price_snapshot || (cancellationFeeMode && defaultTravelRule ? defaultTravelAmount : 0) || 0) || 0),
      ...
      cancellation_fee_rate_type: cancellationFee ? (String(cancellationFeeBasis?.rate_type || "").trim() || "manual") : "",
      cancellation_fee_source_amount: Math.max(0, Number(cancellationFeeBasis?.source_amount || cancellationFee?.unit_price_snapshot || 0) || 0),
      cancellation_fee_manual_reason: cancellationFee ? String(cancellationFeeBasis?.manual_reason || "").trim() : "",
  ...
  const discountPriceRuleId = String(detail?.batch?.discount_price_rule_id || "").trim();
  return {
    visits: visitRows,
...
function buildEditorPayloadFromModel_(model, ruleCatalog, options = {}) {
  ...
  const addLine_ = (priceRuleId, label, unitPrice, qty, options = {}) => {
    ...
    const key = buildBillingLineGroupKey_({ item_type: itemType, price_rule_id: rid, label: lb, unit_price: unit, note });
    if (!lineMap[key]) {
      lineMap[key] = {
  ...
  visits.forEach((v) => {
    ...
    const cancellationMode = !legacyMode && isEditorCancellationFeeMode_(v);
    if (!cancellationMode) {
      if (v.base_rule_id && Number(v.base_amount || 0) > 0) {
        ...
        addLine_(v.base_rule_id, v.base_label || ((ruleCatalog?.byId?.[v.base_rule_id] || {}).label || ""), v.base_amount, 1, { item_type: "visit_base" });
      }
      if (Number(v.parking_amount || 0) > 0) {
        const rid = String(v.parking_rule_id || ((ruleCatalogOptionsByInclusion_(ruleCatalog, "parking_fee", "per_visit") || [])[0] || {}).price_rule_id || "").trim();
        ...
        addLine_(rid, lb, v.parking_amount, 1, { item_type: "parking_fee" });
      }
      if (v.travel_rule_id && Number(v.travel_amount || 0) > 0) {
        ...
        addLine_(v.travel_rule_id, lb, v.travel_amount, 1, { item_type: "travel_fee" });
      }
      if (v.seasonal_rule_id && Number(v.seasonal_amount || 0) > 0) {
        ...
        addLine_(v.seasonal_rule_id, lb, v.seasonal_amount, 1, { item_type: "seasonal_fee" });
      }
    }
    if (legacyMode) {
      if (v.legacy_headcount_rule_id && Number(v.legacy_headcount_amount || 0) > 0) {
        ...
        addLine_(v.legacy_headcount_rule_id, lb, v.legacy_headcount_amount, qty, { item_type: "visit_addon" });
      }
      if (v.legacy_travel_rule_id && Number(v.legacy_travel_amount || 0) > 0) {
        ...
        addLine_(v.legacy_travel_rule_id, lb, v.legacy_travel_amount, 1, { item_type: "travel_fee" });
      }
      if (v.legacy_topping_rule_id && Number(v.legacy_topping_amount || 0) > 0) {
        ...
        addLine_(v.legacy_topping_rule_id, lb, v.legacy_topping_amount, qty, { item_type: "visit_addon" });
      }
    } else {
      if (cancellationMode) {
        ...
        addLine_(rid, lb, v.cancellation_fee_amount, qty, { id: String(v.cancellation_fee_line_item_id || "").trim(), item_type: "cancellation_fee" });
      }
      ...
      extraLines.forEach((line) => {
        ...
        addLine_(rid, lb, line.amount, qty, { id: lineItemId, item_type: (ruleCatalog?.byId?.[rid] || {}).item_type || "custom" });
      });
    }
  });
  if (m.key_pickup_rule_id && Number(m.key_pickup_amount || 0) > 0) {
    ...
    addLine_(m.key_pickup_rule_id, lb, m.key_pickup_amount, qty, { item_type: "key_pickup_fee" });
  }
  if (m.key_return_rule_id && Number(m.key_return_amount || 0) > 0) {
    ...
    addLine_(m.key_return_rule_id, lb, m.key_return_amount, qty, { item_type: "key_return_fee" });
  }
  ...
  const discountPriceRuleId = String(m.discount_price_rule_id || "").trim();
  return {
    items,
    desired_lines: Object.keys(lineMap).map((k) => lineMap[k]),
    cancellation_fee_lines: buildEditorCancellationFeeLines_(visits, legacyMode),
    discount_amount: discountAmount,
...
async function renderInvoicesDetailPage_(app, batchId) {
  ...
  const ruleOptions = (Array.isArray(detail?.price_rules) ? detail.price_rules : [])
    .map((r) => ({
      ...
      label: priceRuleDisplayLabel_(r, String(r?.price_rule_id || "").trim())
    }))
    .filter((x) => x.value);
  try {
  ...
  const mergedRuleOptions = (() => {
    ...
    allRules.forEach((r) => {
      ...
      if (!value) return;
      if (map[value]) return;
      const label = priceRuleDisplayLabel_(r, value);
  ...
  function renderPage_() {
    ...
    const canCreateAdjustment = normalizedBatchStatus === "paid";
    const hasInactiveLinkedVisit = links.some((x) => x && x.visit && x.visit.is_active === false);
    const discountAmount = Math.max(0, Number(batch?.discount_amount || 0) || 0);
    const discountLabel = String(batch?.discount_label || "割引");
    const visitDateById = {};
    ...
    const sourceRows = editing ? workingItems : (Array.isArray(detail?.invoice_items) ? detail.invoice_items.map(cloneEditorItem_) : []);
      const editingTotal = editing
        ? (Array.isArray(editorModel?.visits) ? editorModel.visits.reduce((sum, v) => {
          ...
          const cancellationMode = isEditorCancellationFeeMode_(v);
          const normalSub = Math.max(0, Number(v.base_amount || 0) || 0)
            + Math.max(0, Number(v.parking_amount || 0) || 0)
            + Math.max(0, Number(v.travel_amount || 0) || 0)
            + Math.max(0, Number(v.seasonal_amount || 0) || 0)
            + (legacyMode
              ...
              : editorExtraLinesTotal_(v));
          const cancellationSub = (Math.max(1, Number(v.cancellation_fee_qty || 1) || 1) * Math.max(0, Number(v.cancellation_fee_amount || 0) || 0));
          const sub = cancellationMode ? cancellationSub : normalSub;
          return sum + sub;
        }, 0) : 0)
    ...
    const parkingAutoOptions = ruleCatalogOptionsByInclusion_(ruleCatalog, "parking_fee", "per_visit");
    const travelAutoOptions = ruleCatalogOptionsByInclusion_(ruleCatalog, "travel_fee", "per_visit");
    const seasonalAutoOptions = ruleCatalogOptionsByInclusion_(ruleCatalog, "seasonal_fee", "per_visit");
    const keyPickupAutoOptions = ruleCatalogOptionsByInclusion_(ruleCatalog, "key_pickup_fee", "per_invoice");
    const keyReturnAutoOptions = ruleCatalogOptionsByInclusion_(ruleCatalog, "key_return_fee", "per_invoice");
    const visitOptionOptions = visitOptionLineRuleOptions_(ruleCatalog);
    const editorRowsHtml = editing ? (Array.isArray(editorModel?.visits) ? editorModel.visits.map((v) => {
      const isExcluded = v.excluded === true;
      const cancellationMode = isEditorCancellationFeeMode_(v);
      const normalSubtotal = Math.max(0, Number(v.base_amount || 0) || 0)
        + Math.max(0, Number(v.parking_amount || 0) || 0)
        + Math.max(0, Number(v.travel_amount || 0) || 0)
        + Math.max(0, Number(v.seasonal_amount || 0) || 0)
        + (legacyMode
          ? ((Math.max(1, Number(v.legacy_headcount_qty || 1) || 1) * Math.max(0, Number(v.legacy_headcount_amount || 0) || 0))
            ...
            + (Math.max(1, Number(v.legacy_topping_qty || 1) || 1) * Math.max(0, Number(v.legacy_topping_amount || 0) || 0)))
          : editorExtraLinesTotal_(v));
      const cancellationSubtotal = (Math.max(1, Number(v.cancellation_fee_qty || 1) || 1) * Math.max(0, Number(v.cancellation_fee_amount || 0) || 0));
      const subtotal = cancellationMode ? cancellationSubtotal : normalSubtotal;
      ...
      const showBaseRow = !!String(v.base_rule_id || "").trim() || visitBaseAutoOptions.length > 0;
      const showParkingRow = Math.max(0, Number(v.parking_amount || 0) || 0) > 0 || parkingAutoOptions.length > 0;
      const showTravelRow = !!String(v.travel_rule_id || "").trim() || travelAutoOptions.length > 0;
      const showSeasonalRow = !!String(v.seasonal_rule_id || "").trim() || seasonalAutoOptions.length > 0;
      const visitId = String(v.visit_id || "").trim();
      const visitOptionLines = (Array.isArray(v.extra_lines) ? v.extra_lines : [])
        .map((line, idx) => ({ line, idx }))
        .filter((x) => asItemType_({ price_rule_id: x.line?.rule_id || x.line?.price_rule_id || "" }, ruleCatalog) === "visit_addon");
      const manualExtraLines = (Array.isArray(v.extra_lines) ? v.extra_lines : [])
        .map((line, idx) => ({ line, idx }))
        .filter((x) => asItemType_({ price_rule_id: x.line?.rule_id || x.line?.price_rule_id || "" }, ruleCatalog) !== "visit_addon");
      const openAttr = openEditorVisitIds.has(visitId) ? " open" : "";
      const normalRowStyle = cancellationMode ? " opacity:.45; cursor:not-allowed;" : " cursor:pointer;";
      const showCancellationFeeModeRow = !legacyMode && (cancellationMode || v.is_active === false || v.invoice_reconcile_required === true);
      const cancellationRateType = String(v.cancellation_fee_rate_type || "").trim();
      const cancellationRateLabel = cancellationRateType === "manual" ? "自由入力" : `${cancellationRateType || "未設定"}%`;
      const cancellationModeLabel = cancellationMode ? `キャンセル料金 ${cancellationRateLabel}` : "キャンセル料金に切り替え";
      return `
          ...
          <div style="display:grid; gap:8px; margin-top:8px; padding-left:2px;">
            ...
            ${showCancellationFeeModeRow ? `
              <div data-action="edit-cancellation-fee" data-visit-id="${escapeHtml(v.visit_id)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0; cursor:pointer;">
                <span>🔄️ ${escapeHtml(cancellationModeLabel)}</span>
                <strong>${escapeHtml(cancellationMode ? formatEditorLineAmount_(v.cancellation_fee_amount || 0, v.cancellation_fee_qty || 1) : "")}</strong>
              </div>
              ` : ``}
            ${showBaseRow ? `<div data-action="edit-base" data-visit-id="${escapeHtml(v.visit_id)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0;${normalRowStyle}">
              <span>🔄️ ${escapeHtml(String(v.base_label || "訪問サービス"))}</span>
              <strong>${escapeHtml(amountTextInline_(v.base_amount, "未設定"))}</strong>
            </div>` : ``}
            ${(!legacyMode && visitOptionLines.length) ? visitOptionLines.map(({ line, idx }) => `
              <div data-action="edit-visit-option" data-visit-id="${escapeHtml(v.visit_id)}" data-extra-index="${escapeHtml(String(idx))}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0;${normalRowStyle}">
                <span>🔄️ ${escapeHtml(String(line?.label || "予約オプション"))}</span>
                <strong>${escapeHtml(formatEditorLineAmount_(line?.amount || 0, line?.qty || 1))}</strong>
              </div>
              `).join("") : ``}
            ${(!legacyMode && visitOptionOptions.length) ? `
              <div data-action="add-visit-option" data-visit-id="${escapeHtml(v.visit_id)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0;${normalRowStyle}">
                <span>➕ 予約オプション追加</span>
                <strong>未設定</strong>
              </div>
              ` : ``}
            ${showParkingRow ? `<div data-action="edit-parking" data-visit-id="${escapeHtml(v.visit_id)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0;${normalRowStyle}">
              <span>🔄️ 駐車料金</span>
              <strong>${escapeHtml(amountTextInline_(v.parking_amount, "未設定"))}</strong>
            </div>` : ``}
            ${showTravelRow ? `<div data-action="edit-travel" data-visit-id="${escapeHtml(v.visit_id)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0;${normalRowStyle}">
              <span>🔄️ ${escapeHtml(travelLabel)}</span>
              <strong>${escapeHtml(amountTextInline_(v.travel_amount, "未選択"))}</strong>
            </div>` : ``}
            ${showSeasonalRow ? `<div data-action="edit-seasonal" data-visit-id="${escapeHtml(v.visit_id)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0;${normalRowStyle}">
            ...
            ` : `
              ${manualExtraLines.map(({ line, idx }) => `
              <div data-action="edit-extra" data-visit-id="${escapeHtml(v.visit_id)}" data-extra-index="${escapeHtml(String(idx))}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0;${normalRowStyle}">
                <span>🔄️ ${escapeHtml(String(line?.label || "明細"))}</span>
                <strong>${escapeHtml(formatEditorLineAmount_(line?.amount || 0, line?.qty || 1))}</strong>
              </div>
              `).join("")}
              <div data-action="add-extra" data-visit-id="${escapeHtml(v.visit_id)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0;${normalRowStyle}">
                <span>➕ 明細追加</span>
                <strong>未設定</strong>
              </div>
            `}
        ...
        <div class="row" style="justify-content:flex-end;">
          ...
          <div class="row">
            ${editing
              ...
              : `${canCreateAdjustment ? `<button class="btn btn-ghost" id="btnCreateAdjustmentFromBatch" type="button">追加請求</button>` : ``}${canEditBatch ? `<button class="btn btn-ghost" id="btnStartItemsInline" type="button">請求構成を編集</button>` : ``}`}
          </div>
        </div>
        ${editing ? `
          <div class="card" style="margin-top:10px;">
          ...
          <div style="margin-top:10px;">
            ...
            <div style="margin-top:10px; border-top:1px solid rgba(255,255,255,0.08); padding-top:8px;">
              ${(String(editorModel?.key_pickup_rule_id || "").trim() || keyPickupAutoOptions.length) ? `<div data-action="edit-key-pickup" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0; cursor:pointer;">
                <span>🔄️ 鍵預かり料金</span>
                <strong>${escapeHtml(formatEditorLineAmount_(editorModel?.key_pickup_amount || 0, editorModel?.key_pickup_qty || 1))}</strong>
              </div>` : ``}
              ${(String(editorModel?.key_return_rule_id || "").trim() || keyReturnAutoOptions.length) ? `<div data-action="edit-key-return" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0; cursor:pointer;">
                <span>🔄️ 鍵返却料金</span>
    ...
    qs("#btnSaveItemsInline")?.addEventListener("click", async () => {
      ...
      if (currentBillingStatus !== "paid" && currentSquareStatus && !isSquareDraftLikeStatus_(currentSquareStatus)) {
        const okPublishedReissue = await confirmByModal_(
          "下書き再作成の確認",
          "送信済みのSquare請求書を無効化し、Square請求書下書きを再作成します。本当によろしいですか？",
          "下書きを再作成して保存",
          "戻る"
        );
      ...
      if (!saveConfirmed) {
        const okSave = await confirmByModal_(
          "請求構成保存の確認",
          "現在の請求構成で保存します。よろしいですか？",
          "保存する",
          "編集に戻る"
        );
        if (!okSave) return;
      ...
      try {
        await runWithBlocking_(
          ...
          async () => updateBillingBatchItems_({
            ...
            cancellation_fee_lines: built.cancellation_fee_lines,
    ...
    app.querySelectorAll('[data-action="resync-visit-statuses"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        const bid = String(btn.getAttribute("data-batch-id") || "").trim();
        if (!bid) return;
        const ok = await confirmByModal_(
          "請求ステータスを再同期",
          "Square請求書のステータスを再取得して、請求ステータスを更新します。請求書本体は編集されません。",
          "再同期する",
          "キャンセル"
        );
        if (!ok) return;
        try {
          const out = await runWithBlocking_(
            { title: "請求ステータスを再同期しています", bodyHtml: "Square請求書のステータスを再取得しています...", busyText: "再同期中..." },
            async () => resyncBillingBatchVisitStatuses_(bid)
          );
          await refreshDetail_();
          const updated = Array.isArray(out?.updated_visit_ids) ? out.updated_visit_ids.length : 0;
          const unchanged = Array.isArray(out?.unchanged_visit_ids) ? out.unchanged_visit_ids.length : 0;
          toast({ title: "完了", message: `請求ステータスを再同期しました（予約更新 ${updated}件 / 変更なし ${unchanged}件）。` });
          renderPage_();
        } catch (e) {
    ...
    app.querySelectorAll('[data-action="open-square-invoice"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
    ...
    const findEditorVisit_ = (visitId) => {
      const vid = String(visitId || "").trim();
      if (!vid) return null;
      return (editorModel?.visits || []).find((x) => String(x.visit_id || "") === vid) || null;
    };
    const isNormalEditorActionBlocked_ = (visit) => !legacyMode && isEditorCancellationFeeMode_(visit);
    app.querySelectorAll('[data-action="edit-base"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        const vid = String(btn.getAttribute("data-visit-id") || "").trim();
        const visit = findEditorVisit_(vid);
        if (!visit) return;
        if (isNormalEditorActionBlocked_(visit)) return;
        const options = visitBaseAutoOptions;
    ...
    app.querySelectorAll('[data-action="edit-travel"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        if (!visit) return;
        if (isNormalEditorActionBlocked_(visit)) return;
        const options = travelAutoOptions;
    ...
    app.querySelectorAll('[data-action="edit-seasonal"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        if (!visit) return;
        if (isNormalEditorActionBlocked_(visit)) return;
        const options = seasonalAutoOptions;
    ...
    app.querySelectorAll('[data-action="edit-cancellation-fee"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        const visit = findEditorVisit_(vid);
        if (!visit) return;
        if (isEditorCancellationFeeMode_(visit)) {
          visit.billing_line_mode = "normal";
          ...
          visit.cancellation_fee_rate_type = "";
          visit.cancellation_fee_source_amount = 0;
          visit.cancellation_fee_manual_reason = "";
          openEditorVisitIds.add(vid);
          renderPage_();
          return;
        }
        const sourceAmount = computeEditorVisitNormalSourceAmount_(visit);
        const chosen = await inputCancellationFeeRateByModal_({
          rate_type: visit.cancellation_fee_rate_type || "50",
          source_amount: sourceAmount || visit.cancellation_fee_source_amount || visit.cancellation_fee_amount || 0,
          final_amount: visit.cancellation_fee_amount || 0,
          manual_reason: visit.cancellation_fee_manual_reason || "",
        });
        if (chosen == null) return;
        const amount = Math.max(0, Number(chosen.final_amount || 0) || 0);
        if (!(amount > 0)) {
          visit.billing_line_mode = "normal";
          visit.cancellation_fee_rule_id = "";
          visit.cancellation_fee_line_item_id = "";
          ...
          visit.billing_line_mode = "cancellation_fee";
          visit.cancellation_fee_rule_id = String(visit.cancellation_fee_rule_id || "").trim();
          visit.cancellation_fee_label = "キャンセル料金";
          visit.cancellation_fee_amount = amount;
          visit.cancellation_fee_qty = 1;
          visit.cancellation_fee_rate_type = String(chosen.rate_type || "").trim();
          visit.cancellation_fee_source_amount = Math.max(0, Number(chosen.source_amount || 0) || 0);
          visit.cancellation_fee_manual_reason = String(chosen.manual_reason || "").trim();
        }
        renderPage_();
      });
    });
    app.querySelectorAll('[data-action="edit-extra"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        const chosen = await inputExtraLineByModal_({
          ...
          qty: current.qty || 1,
        }, manualLineRuleOptions_(ruleCatalog));
        if (chosen == null) return;
        ...
        if (!(amount > 0)) {
          ...
          lines[index] = {
            rule_id: String(chosen.rule_id || current.rule_id || "").trim(),
    ...
    app.querySelectorAll('[data-action="edit-visit-option"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        const vid = String(btn.getAttribute("data-visit-id") || "").trim();
        const index = Math.max(0, Number(btn.getAttribute("data-extra-index") || 0) || 0);
        const visit = findEditorVisit_(vid);
        ...
        const current = lines[index] || {};
        const chosen = await inputExtraLineByModal_({
          modal_title: "予約オプション追加",
          allow_free: false,
          rule_id: current.rule_id || "",
          label: current.label || "",
          amount: current.amount || 0,
          qty: current.qty || 1,
        }, visitOptionLineRuleOptions_(ruleCatalog));
        if (chosen == null) return;
        const amount = Math.max(0, Number(chosen.amount || 0) || 0);
        if (!(amount > 0)) {
          lines.splice(index, 1);
    ...
    app.querySelectorAll('[data-action="add-extra"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        if (!visit) return;
        if (isNormalEditorActionBlocked_(visit)) return;
        const chosen = await inputExtraLineByModal_({ label: "", amount: 0, qty: 1 }, manualLineRuleOptions_(ruleCatalog));
        if (chosen == null) return;
        const amount = Math.max(0, Number(chosen.amount || 0) || 0);
        ...
        const lines = Array.isArray(visit.extra_lines) ? visit.extra_lines : [];
        lines.push({
          rule_id: String(chosen.rule_id || "").trim() || "manual:custom",
          line_item_id: "",
          label,
          amount,
          qty: Math.max(1, Number(chosen.qty || 1) || 1),
    ...
    app.querySelectorAll('[data-action="add-visit-option"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        const vid = String(btn.getAttribute("data-visit-id") || "").trim();
        const visit = findEditorVisit_(vid);
        if (!visit) return;
        if (isNormalEditorActionBlocked_(visit)) return;
        const chosen = await inputExtraLineByModal_({ modal_title: "予約オプション追加", allow_free: false, label: "", amount: 0, qty: 1 }, visitOptionLineRuleOptions_(ruleCatalog));
        if (chosen == null) return;
        const amount = Math.max(0, Number(chosen.amount || 0) || 0);
        const label = String(chosen.label || "").trim();
        if (!label || !(amount > 0)) return;
        const lines = Array.isArray(visit.extra_lines) ? visit.extra_lines : [];
        lines.push({
          rule_id: String(chosen.rule_id || "").trim(),
          line_item_id: "",
          label,
          amount,
          qty: Math.max(1, Number(chosen.qty || 1) || 1),
    ...
    app.querySelectorAll('[data-action="edit-key-pickup"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        const options = keyPickupAutoOptions;
    ...
    app.querySelectorAll('[data-action="edit-key-return"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        const options = keyReturnAutoOptions;
    ...
    app.querySelectorAll('[data-action="edit-discount"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        const out = await inputDiscountByModal_({
          price_rule_id: String((editorModel && editorModel.discount_price_rule_id) || "").trim(),
          label: String((editorModel && editorModel.discount_label) || editingDiscountLabel || "割引").trim() || "割引",
          amount: Math.max(0, Number((editorModel && editorModel.discount_amount) || editingDiscountAmount || 0) || 0),
        }, discountOptions);
        if (!out) return;
        const amount = Math.max(0, Number(out.amount || 0) || 0);
        const discountPriceRuleId = String(out.price_rule_id || "").trim();
