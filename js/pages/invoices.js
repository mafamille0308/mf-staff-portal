import { render, qs, escapeHtml, toast } from "../ui.js";
...
async function renderInvoicesDetailPage_(app, batchId) {
  ...
  function renderPage_() {
    ...
    const batch = detail?.batch || {};
    const batchType = String(batch?.batch_type || "invoice").trim().toLowerCase() || "invoice";
    const isCancellationBatch = batchType === "cancellation";
    const isPaidBatch = String(batch?.billing_status || "").trim().toLowerCase() === "paid";
    if (isCancellationBatch && editing) editing = false;
    const discountAmount = Math.max(0, Number(batch?.discount_amount || 0) || 0);
    const discountLabel = String(batch?.discount_label || "割引");
    const visitDateById = {};
    links.forEach((link) => {
        ...
        <div class="row row-between">
          ...
          <div class="row">
            ${editing
              ...
              : (isCancellationBatch
                ? `<button class="btn btn-ghost" id="btnReissueCancellation" type="button" ${isPaidBatch ? "disabled" : ""}>キャンセル請求を再発行</button>`
                : `<button class="btn btn-ghost" id="btnStartItemsInline" type="button">請求構成を編集</button>`)}
          </div>
        </div>
        ${(!editing && isCancellationBatch && isPaidBatch) ? `<div class="p" style="margin-top:8px; color:#ffd4a3;">入金済みのため再発行できません。返金後に再実行してください。</div>` : ``}
        ${editing ? `
          <div class="card" style="margin-top:10px;">
            <div class="p">
    ...
    app.querySelectorAll('[data-action="edit-discount"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        const currentDiscountRule = discountOptions.find((x) => String(x?.label || "").trim() === String((editorModel && editorModel.discount_label) || editingDiscountLabel || "割引").trim()) || null;
        const currentDiscountRuleId = String(currentDiscountRule?.price_rule_id || "").trim();
        const discountOptionsHtml = discountOptions.map((x) => {
          const rid = String(x?.price_rule_id || "").trim();
          const isSelected = currentDiscountRuleId && rid === currentDiscountRuleId;
        ...
        const out = await openEditorModal_({
          ...
          `,
          onSubmit: (root) => {
            const templateId = String(root.querySelector('[data-el="template"]')?.value || "").trim();
            const template = discountOptions.find((x) => String(x?.price_rule_id || "").trim() === templateId) || null;
