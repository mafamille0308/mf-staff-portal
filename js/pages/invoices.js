...
function buildEditorModel_(detail, links, ruleCatalog, options = {}) {
  ...
  const visitRows = links.map((link) => {
    ...
    return {
      ...
      parking_rule_id: String(parking?.price_rule_id || ((ruleCatalogOptionsByInclusion_(ruleCatalog, "parking_fee", "per_visit") || [])[0] || {}).price_rule_id || "").trim(),
      ...
      return `
        <details class="details-compact invoice-edit-details" data-visit-id="${escapeHtml(visitId)}"${openAttr}>
          ...
          <div style="display:grid; gap:8px; margin-top:8px; padding-left:2px;">
            ...
            ` : `
            ${showBaseRow ? `<div data-action="edit-base" data-visit-id="${escapeHtml(v.visit_id)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0; cursor:pointer;">
            ...
            ${(!legacyMode && visitOptionLines.length) ? visitOptionLines.map(({ line, idx }) => `
              <div data-action="edit-visit-option" data-visit-id="${escapeHtml(v.visit_id)}" data-extra-index="${escapeHtml(String(idx))}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0; cursor:pointer;">
            ...
            ${(!legacyMode && visitOptionOptions.length) ? `
              <div data-action="add-visit-option" data-visit-id="${escapeHtml(v.visit_id)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0; cursor:pointer;">
                <span>➕ 予約オプション追加</span>
                <strong>未設定</strong>
              </div>
              ` : ``}
            ${showParkingRow ? `<div data-action="edit-parking" data-visit-id="${escapeHtml(v.visit_id)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0; cursor:pointer;">
              <span>🔄️ 駐車料金</span>
              <strong>${escapeHtml(amountTextInline_(v.parking_amount, "未設定"))}</strong>
            </div>` : ``}
            ${showTravelRow ? `<div data-action="edit-travel" data-visit-id="${escapeHtml(v.visit_id)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0; cursor:pointer;">
              <span>🔄️ ${escapeHtml(travelLabel)}</span>
              <strong>${escapeHtml(amountTextInline_(v.travel_amount, "未選択"))}</strong>
            </div>` : ``}
            ${showSeasonalRow ? `<div data-action="edit-seasonal" data-visit-id="${escapeHtml(v.visit_id)}" style="display:flex; align-items:center; justify-content:space-between; width:100%; padding:6px 0; cursor:pointer;">
