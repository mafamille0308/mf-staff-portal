...
function isMeetingVisit_(visit) {
  const p = String((visit && visit.product_name) || "").toLowerCase();
  if (p.indexOf("カウンセリング") >= 0 || p.indexOf("打ち合わせ") >= 0) return true;
...
function productBadgeLabel_(visit) {
  return String((visit && (visit.product_name || visit.service_name || visit.visit_type)) || "").trim() || "-";
}
    ...
    if (action === "change-visit-type") {
      ...
      const applyFinal = (u) => {
        ...
        if (vt) {
          ...
          actEl.textContent = productBadgeLabel_(Object.assign({}, visit, uu, { visit_type: vt }));
        }
