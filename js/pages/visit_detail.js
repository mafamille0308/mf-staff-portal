...
import { portalMeetingNotificationManualStatusUpdate_ } from "./portal_api.js";
...
function actionIconSvg_(name) {
  ...
  if (name === "clipboard-type") {
    return `<svg ${common}><rect x="8" y="2" width="8" height="4" rx="1"></rect><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"></path><path d="M9 12h6"></path><path d="M12 12v6"></path></svg>`;
  }
  return "";
}
...
function deriveRefundFollowupStatus_(visit, billingStatus, isActive) {
  const v = visit || {};
  ...
  if (refundDetected) {
    ...
    return refundKind === "partial" ? "refunded_partial" : "refunded_full";
  }
...
export async function renderVisitDetail(appEl, query) {
  ...
  render(appEl, `
    <section class="section">
      <div class="row" style="justify-content:space-between; align-items:center;">
        <h1 class="h1">予約詳細</h1>
        <a class="btn btn-icon-action" href="#/visits" id="btnBackToList" title="一覧に戻る" aria-label="一覧に戻る">${actionIconSvg_("back")}</a>
      </div>
      <div class="hr"></div>
  ...
  const refundFollowupStatus = deriveRefundFollowupStatus_(visit, billingStatus, isActive);
  const refundLabel = refundFollowupLabel_(refundFollowupStatus);
  const refundBadgeClass = refundFollowupBadgeClass_(refundFollowupStatus);
  const paidTimeChanged = visit.paid_time_change_detected === true || String(visit.paid_time_change_detected || "").toLowerCase() === "true";
  const paidTimeChangeBefore = [
    fmtDateTimeJst(visit.paid_time_change_before_start_time || ""),
    fmtDateTimeJst(visit.paid_time_change_before_end_time || "")
  ].filter((x) => x && x !== "-").join(" - ");
  const meetingNotifyStatus = normalizeMeetingNotifyStatus_(visit.meeting_notify_status);
  ...
  const customerNameWithHonorific = withCustomerHonorific_(customerName);
  ...
  const visitHtml = `
    ...
    >
      ...
      ` : ``}
      <span class="badge badge-billing-status ${billingStatusStageClass_(billingStatus)}"
      ...
      ${hasCancellationFee ? `<span class="badge badge-danger">キャンセル料: ${escapeHtml(formatMoney_(cancellationFeeAmount))}円</span>` : ``}
        ${meetingNotifyLabel ? `<span class="${meetingNotifyBadgeClass_(meetingNotifyStatus)}" data-action="toggle-meeting-notify" style="cursor:pointer;" title="タップで通知確認ステータスを切り替え">${escapeHtml(meetingNotifyLabel)}</span>` : ``}
        ...
        >${isActive ? "有効" : "キャンセル"}</span>
      </div>
      <div class="hr"></div>
      <div class="p">
      ${paidTimeChanged ? `
      <div class="p" style="color:var(--state-warn-fg); margin-bottom:8px;">
        <strong>支払後日時変更あり</strong><br>
        変更前：${escapeHtml(paidTimeChangeBefore || "変更前日時不明")}
      </div>
      ` : ``}
      ...
      <div><strong>終了</strong>：<span id="vdEndTimeText">${escapeHtml(displayOrDash(endDisp))}</span></div>
        ...
        <div class="field field-customer"><strong>顧客名</strong>：${escapeHtml(customerNameWithHonorific || displayOrDash(visit.customer_id))}</div>
        ...
        <div class="vd-action-stack">
          <div class="vd-memo-action">
            <button class="btn btn-icon-action" type="button" id="btnEditMemo" title="メモを編集" aria-label="メモを編集">${actionIconSvg_("memo")}</button>
            <div id="memoEditBox" class="is-hidden" style="margin-top:10px;">
              <textarea id="memoInput" rows="5" style="width:100%;" placeholder="空欄で保存するとメモは空になります。"></textarea>
          ...
          ${isMeetingVisit_(visit) ? `<button class="btn btn-icon-label" type="button" id="btnMeetingCustomer" title="打ち合わせフォーム" aria-label="打ち合わせフォーム">${actionIconSvg_("clipboard-type")}<span>打ち合わせフォーム</span></button>` : ``}
        </div>
      </div>
    </div>
  `;
  ...
  try {
    ...
    if (customerDetail && customerDetail.customer) {
      ...
      const customerLabel2Raw = String(c.name || customerName || "").trim();
      const customerLabel2 = withCustomerHonorific_(customerLabel2Raw);
      ...
      customerInfoHtml = `
        <div class="card">
          <div class="row row-between">
            ...
            <div>
              ${customerDetailHref ? `<a class="btn btn-icon-action" href="${customerDetailHref}" title="顧客詳細へ" aria-label="顧客詳細へ">${actionIconSvg_("customer")}</a>` : ``}
            </div>
          </div>
          <div class="hr"></div>
          <div class="p">
        ...
        ? pets.map(p => `
            <div class="card">
              <div class="p">
                ...
                ${(() => {
                  const genderRaw = fmt(p.gender || p.sex || "");
                  const gender = genderRaw.includes("オス") ? "オス" : (genderRaw.includes("メス") ? "メス" : "");
                  const neutered = (p.neutered === true || String(p.neutered || "").toLowerCase() === "true" || /去勢|避妊/.test(genderRaw));
                  return `<div>性別：${escapeHtml(displayOrDash(gender))}</div><div>去勢・避妊：${escapeHtml(neutered ? "済" : "未")}</div>`;
                })()}
  ...
  host.addEventListener("click", async (e) => {
    ...
    if (action === "toggle-active") {
      ...
      const currentBillingStatus = normalizeCancelBillingStatus_(rootCard?.dataset?.billingStatus || "");
      ...
      try {
        ...
        if (!nextActive) {
          ...
          toast({
            title: "更新完了",
            message: done.invoice_reconcile_required === true
    ...
    if (action === "toggle-meeting-notify") {
      if (actEl.dataset.busy === "1") return;
      const currentStatus = normalizeMeetingNotifyStatus_(rootCard?.dataset?.meetingNotifyStatus || "");
      ...
      try {
        ...
        if (rootCard) rootCard.dataset.meetingNotifyStatus = nextStatus;
        actEl.textContent = meetingNotifyBadgeLabel_(nextStatus);
        actEl.className = meetingNotifyBadgeClass_(nextStatus);
        toast({ title: "更新完了", message: "通知確認ステータスを更新しました。" });
        markVisitsListDirty_();
      } catch (err) {
        if (rootCard) rootCard.dataset.meetingNotifyStatus = prevStatus;
        actEl.textContent = prevText;
    ...
    if (action === "change-price-rule") {
      ...
      try {
        const isDone = rootCard?.dataset?.done === "1";
        if (isDone) {
