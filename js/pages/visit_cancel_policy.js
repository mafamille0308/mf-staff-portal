...
export function buildCancelPolicyMessage_(preview, options = {}) {
  ...
  const rawStatus = String(preview?.billing_status || "").trim().toLowerCase();
  const isDraftStatus = ["draft", "invoice_draft", "pending", "unpaid"].includes(rawStatus);
  const action = String(preview?.action || "").trim().toLowerCase();
  ...
  if (action === "already_cancelled") {
    if (status === "paid") return "この予約はすでに削除済みです（支払い済み）。Square側の返金状況を確認してください。";
    return "この予約はすでに削除済みです。";
  }
  if (isDraftStatus && remainingVisitCount > 0 && (rate === 50 || rate === 100)) {
    return "下書き請求書を再構成し、残存予約+キャンセル料を反映します。";
  }
  if (isDraftStatus && rate === 0) return remainingVisitCount > 0
    ? "下書き請求書から対象予約を除外して再構成します。"
    : "下書き請求書を削除し、予約をキャンセルします。";
  if (isDraftStatus && rate > 0) return `下書き請求書を再構成し、キャンセル料 ${formatMoney(feeAmount)}円 を反映します。`;
  ...
  if (status === "paid" && rate === 0) return "Square側でキャンセル予約分の全額返金が必要です。訪問基本・繁忙期加算・駐車・出張料金：100％ / 鍵系は実態に応じて返金判断してください。";
  if (status === "paid" && rate === 50) return "Square側でキャンセル予約分の一部返金が必要です。訪問基本・繁忙期加算：50％ / 駐車・出張料金：100％ / 鍵系は実態に応じて返金判断してください。";
  if (status === "paid" && rate === 100) return "Square側でキャンセル予約分の一部返金が必要な可能性があります。駐車・出張料金：100％ / 鍵系は実態に応じて返金判断してください。";
  return "予約を無効にします。";
}
