...
function inputRow_(label, name, value, { type = "text", placeholder = "", help = "", readonly = false, min = "", step = "" } = {}) {
  ...
  return `
    <div class="p" style="margin-bottom:10px;">
      <div style="opacity:.85; margin-bottom:4px;"><strong>${escapeHtml(label)}</strong></div>
      <input class="input" type="${escapeHtml(type)}" name="${escapeHtml(name)}" value="${escapeHtml(normStr_(value))}" placeholder="${escapeHtml(placeholder)}" ${ro} ${minAttr} ${stepAttr} ${inputmodeAttr}/>
...
export async function renderCustomerDetail(appEl, query) {
  ...
  host.addEventListener("click", async (e) => {
    ...
    if (a === "care:save") {
      ...
      try {
        _busy = true;
        const resUp0 = await callGas(patch, idToken);
  ...
  function renderHost_(detail) {
    ...
    const careActionsHtml = (_careMode === "edit")
      ? `
          <button class="btn btn-ghost" type="button" data-action="care:cancel-edit" ${_busy ? "disabled" : ""}>キャンセル</button>
          <button class="btn" type="button" data-action="care:save" ${_busy ? "disabled" : ""}>保存</button>
        `
      : `<button class="btn" type="button" data-action="care:enter-edit" ${_busy ? "disabled" : ""}>編集</button>`;
  ...
  try {
    ...
    _careMode = "view";
    try {
      const rr = await callGas({ action: "listBillingPriceRules", only_active: true }, idToken);
