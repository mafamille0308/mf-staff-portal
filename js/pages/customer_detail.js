...
const FIELD_LABELS_JA = {
  ...
  lock_no: "ロック番号",
  notes: "メモ",
  customer_fee_settings: "請求に反映する料金",
  stage: "ステージ",
  registered_date: "登録日",
  updated_at: "更新日時",
...
function findCustomerFeeSettingForField_(customer, field) {
  ...
  return rows.find((row) => {
    ...
    const feeRole = String(row?.fee_role || "").trim();
    return categoryKey === field.category_key && feeRole === field.fee_role;
  }) || null;
}

function customerFeeDisplayValue_(customer, field, optionsByRole) {
  const setting = findCustomerFeeSettingForField_(customer, field);
  if (setting) {
    const mode = String(setting.value_mode || "").trim();
    if (mode === "fixed_amount") {
      const amount = Math.max(0, Number(setting.amount_override || 0) || 0);
      return amount > 0 ? `${formatMoney_(amount)}円` : "設定しない";
    }
    const rid = String(setting.price_rule_id || "").trim();
    const option = (Array.isArray(optionsByRole?.[field.role]) ? optionsByRole[field.role] : [])
      .find((x) => String(x.price_rule_id || "").trim() === rid);
    if (option) return `${option.label}（${formatMoney_(option.amount || 0)}円）`;
    return rid ? "商品マスタから選択" : "設定しない";
  }
  if (field.role === "parking") {
    const amount = Math.max(0, Number(customer?.parking_fee_amount || customer?.parkingFeeAmount || 0) || 0);
    return amount > 0 ? `${formatMoney_(amount)}円` : "設定しない";
  }
  if (field.role === "business_trip") {
    const amount = Math.max(0, Number(customer?.travel_fee_amount || customer?.travelFeeAmount || 0) || 0);
    return amount > 0 ? `${formatMoney_(amount)}円` : "設定しない";
  }
  if (field.role === "key_pickup") {
    return keyFeeRuleLabel_(customer?.key_pickup_fee_rule || customer?.keyPickupFeeRule) === "有料" ? "有料" : "設定しない";
  }
  if (field.role === "key_return") {
    return keyFeeRuleLabel_(customer?.key_return_fee_rule || customer?.keyReturnFeeRule) === "有料" ? "有料" : "設定しない";
  }
  return "設定しない";
}

function customerFeeSummaryHtml_(customer, optionsByRole) {
  return CUSTOMER_FEE_FIELDS_.map((field) => `
    <div><strong>${escapeHtml(field.label)}</strong>：${escapeHtml(customerFeeDisplayValue_(customer, field, optionsByRole))}</div>
  `).join("");
}
...
function readCustomerFeeSettingsFromForm_(formEl) {
  ...
  const legacy = {
    ...
    travel_fee_amount: 0,
    key_pickup_fee_rule: "free",
    key_return_fee_rule: "free",
  };
  CUSTOMER_FEE_FIELDS_.forEach((field) => {
    const role = field.role;
...
export async function renderCustomerDetail(appEl, query) {
  ...
  let _customerFeeOptions = normalizeCustomerFeeRuleOptionsByRole_([]);
    ...
    <section class="section">
      <div class="row row-between">
        ...
        <div class="row" style="gap:8px;">
          ${canManageCustomerAssignments_() ? `<button class="btn btn-icon-action" id="btnAssignments" type="button" title="担当追加/終了" aria-label="担当追加/終了">${actionIconSvg_("assign")}</button>` : ``}
          ...
          <a class="btn btn-icon-action" id="btnCustomerVisits" href="#/visits?customer_id=${encodeURIComponent(customerId)}&scope=future&back_to=${encodeURIComponent(location.hash || `#/customers?id=${encodeURIComponent(customerId)}`)}" title="予約一覧へ" aria-label="予約一覧へ">${actionIconSvg_("visits")}</a>
  ...
  const assignmentsBtn = appEl.querySelector("#btnAssignments");
  assignmentsBtn?.addEventListener("click", async () => {
    const cname = String((_detail && _detail.customer && _detail.customer.name) || "").trim();
    await openCustomerAssignmentModal({ customerId, customerName: cname, idToken });
  });
  ...
  host.addEventListener("click", async (e) => {
    ...
    if (a === "cd:save") {
      ...
      patch.customer.customer_fee_settings = feeSettings.rows;
      setIfChanged("parking_fee_amount", String(feeSettings.legacy.parking_fee_amount), String(c0.parking_fee_amount ?? 0));
      setIfChanged("travel_fee_amount", String(feeSettings.legacy.travel_fee_amount), String(c0.travel_fee_amount ?? 0));
      ...
      const nextKpfr = feeSettings.legacy.key_pickup_fee_rule;
      const nextKrfr = feeSettings.legacy.key_return_fee_rule;
      ...
      if (nextKpfr !== curKpfr) patch.customer.key_pickup_fee_rule = nextKpfr;
      if (nextKrfr !== curKrfr) patch.customer.key_return_fee_rule = nextKrfr;
  ...
  host.addEventListener("change", (e) => {
    ...
    if (t.matches('input[type="radio"][name^="fee_mode_"]')) {
      syncCustomerFeeSettingFields_(host.querySelector('form[data-el="customerEditForm"]'));
    }
  });
  ...
  function renderCustomerViewBodyHtml_(c) {
    return `
      <div class="p">
        ...
        <div class="hr"></div>
        <div class="fw-900">${escapeHtml(FIELD_LABELS_JA.customer_fee_settings)}</div>
        ${customerFeeSummaryHtml_(c, _customerFeeOptions)}
      ...
      <form data-el="customerEditForm">
        <div class="card" style="margin-top:12px;">
          ...
          <div class="p">
            ...
            ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[0], _customerFeeOptions.parking, c.parking_fee_amount || c.parkingFeeAmount || 0)}
            ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[1], _customerFeeOptions.business_trip, c.travel_fee_amount || c.travelFeeAmount || 0)}
            ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[2], _customerFeeOptions.travel, 0)}
            ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[3], _customerFeeOptions.key_pickup, 0, kpfr === "paid" || normStr_(c.key_pickup_fee_rule || c.keyPickupFeeRule) === "有料")}
            ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[4], _customerFeeOptions.key_return, 0, krfr === "paid" || normStr_(c.key_return_fee_rule || c.keyReturnFeeRule) === "有料")}
            <div class="p" style="margin-bottom:10px;">
  ...
  function renderHost_(detail) {
    ...
    try {
      const regBtn = appEl.querySelector("#btnRegister");
      const visitsBtn = appEl.querySelector("#btnCustomerVisits");
      if (regBtn) {
        ...
        if (visitsBtn) {
          ...
          params.set("back_to", location.hash || `#/customers?id=${encodeURIComponent(customerId)}`);
          visitsBtn.setAttribute("href", `#/visits?${params.toString()}`);
        }
      }
    } catch (_) {}
  ...
  try {
    ...
    try {
      ...
      _customerFeeOptions = normalizeCustomerFeeRuleOptionsByRole_(rules);
    } catch (_) {
      _customerFeeOptions = normalizeCustomerFeeRuleOptionsByRole_([]);
    }
    renderHost_(detail);
