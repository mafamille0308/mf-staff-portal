...
export async function renderMeetingCustomerForm(appEl, query) {
  ...
  const matchedTravel = travelFeeOptions.find((x) => Number(x.amount || 0) === curTravelAmount) || null;
  ...
  const collectDraft_ = () => {
    ...
    const fieldNames = [
      ...
      "key_pickup_rule", "key_pickup_rule_other", "key_pickup_fee_rule",
      "key_return_rule", "key_return_rule_other", "key_return_fee_rule",
      "key_location", "lock_no", "parking_info", "parking_fee_amount", "travel_fee_rule_pick", "notes",
    ];
    const values = {};
  ...
  saveBtn?.addEventListener("click", async () => {
    ...
    const customer = {
      ...
      parking_fee_amount: getFormValue_(formEl, "parking_fee_amount"),
      travel_fee_amount: (() => {
        const raw = String(getFormValue_(formEl, "travel_fee_rule_pick") || "").trim();
        if (!raw) return "0";
        const parts = raw.split("|");
        return String(Math.max(0, Number(parts[1] || 0) || 0));
      })(),
