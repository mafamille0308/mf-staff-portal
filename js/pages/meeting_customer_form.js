import { render, toast, escapeHtml, fmt, displayOrDash, fmtDateTimeJst, showModal } from "../ui.js";
...
import { getMeetingCustomerProfilePolicy_, submitMeetingCustomerProfilePolicy_ } from "./meeting_customer_profile_policy.js";
...
function getFormValue_(formEl, name) {
  ...
  if (checked) return normStr_(checked.value);
...
function normalizeCustomerFeeRuleOptionsByRole_(rows) {
...
function findCustomerFeeSettingForField_(customer, field) {
  const rows = Array.isArray(customer?.customer_fee_settings) ? customer.customer_fee_settings : [];
  return rows.find((row) => (
    String(row?.category_key || "").trim() === field.category_key
    && String(row?.fee_role || "").trim() === field.fee_role
  )) || null;
}

function customerFeeFieldHtml_(field, options, currentAmount, currentEnabled = false, currentSetting = null) {
  const role = String(field?.role || "").trim();
  const settingMode = String(currentSetting?.value_mode || "").trim();
  const settingRuleId = String(currentSetting?.price_rule_id || "").trim();
  const amount = settingMode === "fixed_amount"
    ? Math.max(0, Number(currentSetting?.amount_override || 0) || 0)
    : Math.max(0, Number(currentAmount || 0) || 0);
  const optionList = Array.isArray(options) ? [...options] : [];
  if (settingRuleId && !optionList.some((x) => x.price_rule_id === settingRuleId)) {
    optionList.push({ price_rule_id: settingRuleId, label: "現在の商品（マスタ候補外）", amount });
  }
  const representative = optionList.find((x) => x.price_rule_id === settingRuleId) || optionList[0] || null;
  const matched = settingMode === "master_price" && settingRuleId
    ? optionList.find((x) => x.price_rule_id === settingRuleId)
    : optionList.find((x) => amount > 0 && Number(x.amount || 0) === amount) || (currentEnabled ? optionList[0] : null);
  const mode = settingMode === "fixed_amount" ? "fixed_amount" : (matched ? "master_price" : (amount > 0 || currentEnabled ? "fixed_amount" : "none"));
  ...
  return `
    <div class="panel-soft mt-10" data-customer-fee-field="${escapeHtml(role)}"${representativeAttr}>
      ...
      <div class="grid-8 mt-8">
        <label class="row gap-8"><input type="radio" name="fee_mode_${escapeHtml(role)}" value="none" ${mode === "none" ? "checked" : ""}>設定しない</label>
        <label class="row gap-8"><input type="radio" name="fee_mode_${escapeHtml(role)}" value="master_price" ${mode === "master_price" ? "checked" : ""}>商品マスタから選択</label>
        <select class="input" name="fee_rule_${escapeHtml(role)}" data-fee-mode-section="${escapeHtml(role)}:master_price">
          <option value="">選択してください</option>
          ${optionList.map((o) => `<option value="${escapeHtml(String(o.price_rule_id || ""))}|${escapeHtml(String(Math.max(0, Number(o.amount || 0) || 0)))}" ${matched && matched.price_rule_id === o.price_rule_id ? "selected" : ""}>${escapeHtml(String(o.label || ""))}（${escapeHtml(formatMoney_(o.amount || 0))}円）</option>`).join("")}
        </select>
        <label class="row gap-8"><input type="radio" name="fee_mode_${escapeHtml(role)}" value="fixed_amount" ${mode === "fixed_amount" ? "checked" : ""}>金額入力</label>
        <input class="input" name="fee_amount_${escapeHtml(role)}" data-fee-mode-section="${escapeHtml(role)}:fixed_amount" type="number" min="0" step="1" inputmode="numeric" value="${escapeHtml(String(amount || ""))}" placeholder="例：500" />
...
export async function renderMeetingCustomerForm(appEl, query) {
  ...
  let detail = null;
  let customerFeeOptions = normalizeCustomerFeeRuleOptionsByRole_([]);
  try {
    const res = await runWithBlocking_(
      ...
      async () => await getMeetingCustomerProfilePolicy_(visitId, idToken)
    );
    if (!res || res.success === false) throw new Error((res && (res.error || res.message)) || "getMeetingCustomerProfile failed");
    detail = res;
    try {
      const rulesRes = await listBillingPriceRulesPolicy_(idToken, true);
      const rules = Array.isArray(rulesRes?.results) ? rulesRes.results : (Array.isArray(rulesRes) ? rulesRes : []);
      customerFeeOptions = normalizeCustomerFeeRuleOptionsByRole_(rules);
    } catch (_) {
      customerFeeOptions = normalizeCustomerFeeRuleOptionsByRole_([]);
    }
  } catch (e) {
  ...
  const customerNameWithHonorific = withCustomerHonorific_(c.name || visit.customer_name);

  host.innerHTML = `
    <div class="card">
      <div class="p">
        <div><strong>予約ID</strong>：${escapeHtml(displayOrDash(visit.visit_id))}</div>
        <div><strong>商品</strong>：${escapeHtml(displayOrDash(visit.price_rule_label || visit.product_name || visit.service_name || visit.price_rule_id))}</div>
        <div><strong>打ち合わせ日時</strong>：${escapeHtml(displayOrDash(fmtDateTimeJst(visit.start_time || "")))}</div>
        <div><strong>顧客名</strong>：${escapeHtml(displayOrDash(customerNameWithHonorific))}</div>
    ...
    <form data-el="meetingCustomerForm">
      <div class="card" style="margin-top:12px;">
        ...
        <div class="p" style="margin-top:8px;">
          ...
          ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[0], customerFeeOptions.parking, 0, false, findCustomerFeeSettingForField_(c, CUSTOMER_FEE_FIELDS_[0]))}
          ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[1], customerFeeOptions.business_trip, 0, false, findCustomerFeeSettingForField_(c, CUSTOMER_FEE_FIELDS_[1]))}
          ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[2], customerFeeOptions.travel, 0, false, findCustomerFeeSettingForField_(c, CUSTOMER_FEE_FIELDS_[2]))}
          ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[3], customerFeeOptions.key_pickup, 0, false, findCustomerFeeSettingForField_(c, CUSTOMER_FEE_FIELDS_[3]))}
          ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[4], customerFeeOptions.key_return, 0, false, findCustomerFeeSettingForField_(c, CUSTOMER_FEE_FIELDS_[4]))}
          <div class="p" style="margin-bottom:10px;">
  ...
  const draftKey = `mf_meeting_customer_form_draft_${visitId}`;
  const collectDraft_ = () => {
    ...
    const fieldNames = [
      ...
      "key_pickup_rule", "key_pickup_rule_other",
      "key_return_rule", "key_return_rule_other",
      "key_location", "lock_no", "parking_info", "notes",
    ];
    CUSTOMER_FEE_FIELDS_.forEach((field) => {
      fieldNames.push(`fee_mode_${field.role}`, `fee_rule_${field.role}`, `fee_amount_${field.role}`);
    });
    const values = {};
    fieldNames.forEach((name) => { values[name] = getFormValue_(formEl, name); });
    values.terms_agreed = !!formEl.querySelector('input[name="terms_agreed"]')?.checked;
    return values;
  };
  const persistDraft_ = () => {
    try {
      const values = collectDraft_();
      if (!values) return;
  ...
  const restoreDraftIfAny_ = async () => {
    try {
      const raw = localStorage.getItem(draftKey);
      if (!raw || !formEl) return;
      const parsed = JSON.parse(raw);
      const values = (parsed && parsed.values && typeof parsed.values === "object") ? parsed.values : null;
      if (!values) return;
      const ok = await showModal({
        title: "下書きを復元",
        bodyHtml: `<div class="p">この予約の未保存下書きがあります。復元しますか？</div>`,
        okText: "復元",
        cancelText: "破棄",
      });
      if (!ok) {
      ...
      Object.keys(values).forEach((name) => {
        if (name === "terms_agreed") return;
        const checked = formEl.querySelector(`input[type="radio"][name="${CSS.escape(name)}"][value="${CSS.escape(String(values[name] || ""))}"]`);
        if (checked) {
          checked.checked = true;
          restored += 1;
          return;
        }
        const named = formEl.elements && formEl.elements.namedItem ? formEl.elements.namedItem(name) : null;
        const el = (named && named.nodeType === 1) ? named : null;
        if (!el) return;
        const next = String(values[name] == null ? "" : values[name]);
        if (String(el.value || "") !== next) {
          el.value = next;
          restored += 1;
        }
      });
      const termsEl = formEl.querySelector('input[name="terms_agreed"]');
      if (termsEl && !!termsEl.checked !== !!values.terms_agreed) {
        termsEl.checked = !!values.terms_agreed;
        restored += 1;
      }
      toast({ title: "下書き復元", message: `${restored}項目を復元しました。` });
      syncCustomerFeeSettingFields_(formEl);
    } catch (e) {
      console.warn("[meeting-form] restore draft failed", String(e?.message || e));
      toast({ title: "下書き復元失敗", message: "下書きの復元に失敗しました。再ログイン後に再度お試しください。" });
    }
  };
  const syncOtherRuleInputState_ = (selectName, otherName) => {
  ...
  syncOtherRuleInputState_("key_pickup_rule", "key_pickup_rule_other");
  syncOtherRuleInputState_("key_return_rule", "key_return_rule_other");
  syncCustomerFeeSettingFields_(formEl);
  formEl?.addEventListener("change", (e) => {
    ...
    if (t.matches('input[type="radio"][name^="fee_mode_"]')) {
      syncCustomerFeeSettingFields_(formEl);
    }
  });
  ...
  saveBtn?.addEventListener("click", async () => {
    ...
    const customer = {
      ...
      customer_fee_settings: feeSettings.rows,
    ...
    const ok = await showModal({
      ...
      bodyHtml: `
        ...
        <div class="hr"></div>
        <div class="p text-sm" style="opacity:.75;">保存後、顧客情報を更新し、同意監査ログと通知処理を実行します。</div>
      `,
      okText: "保存",
      cancelText: "キャンセル",
    });
    ...
    try {
      ...
      const res = await runWithBlocking_(
        {
          title: "顧客情報を保存しています",
          bodyHtml: "保存・同意監査ログ記録・通知処理を実行しています。",
          busyText: "保存中...",
        },
        async () => await submitMeetingCustomerProfilePolicy_({
          visit_id: visitId,
          customer,
          terms_agreed: true,
        }, idToken)
      ...
      const notify = (res && res.notification && typeof res.notification === "object") ? res.notification : null;
      if (notify && notify.attempted === true && notify.success === false) {
        toast({ title: "通知失敗", message: "保存は完了しましたが、通知送信に失敗しました。管理者に連絡してください。" });
      }
      if (notify && notify.manual_send_required === true) {
        const manualReason = String(notify.manual_send_reason || "").trim();
        const settingsHref = "#/settings?area=business";
        const settingsGuideHtml = manualReason === "reply_to_missing"
          ? `
            <div class="hr"></div>
            <div class="p text-sm">設定場所：設定 → 事業 → 店舗 → 店舗設定 → メールアドレス</div>
          `
          : "";
        const openSettings = await showModal({
          title: "手動通知が必要です",
          bodyHtml: `
            <div class="p">${escapeHtml(String(notify.manual_send_hint || "メールアドレスが未登録のため自動通知は送信されていません。LINEなどから通知を送信してください。"))}</div>
            ${settingsGuideHtml}
            <div class="hr"></div>
            <div class="p text-sm" style="opacity:.75;">通知キューには manual_send_required として登録済みです。</div>
          `,
          okText: manualReason === "reply_to_missing" ? "店舗設定を開く" : "確認",
          cancelText: manualReason === "reply_to_missing" ? "閉じる" : null,
        });
        if (manualReason === "reply_to_missing" && openSettings) {
          toast({ title: "保存完了", message: "顧客情報を更新しました。" });
          location.hash = settingsHref;
          return;
        }
      }
      toast({ title: "保存完了", message: "顧客情報を更新しました。" });
