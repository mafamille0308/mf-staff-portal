...
function customerFeeFieldHtml_(field, options, currentAmount, currentEnabled = false) {
  const role = String(field?.role || "").trim();
  const amount = Math.max(0, Number(currentAmount || 0) || 0);
  const optionList = Array.isArray(options) ? options : [];
  const representative = optionList[0] || null;
  const matched = optionList.find((x) => amount > 0 && Number(x.amount || 0) === amount) || (currentEnabled ? optionList[0] : null);
  const mode = matched ? "master_price" : (amount > 0 || currentEnabled ? "fixed_amount" : "none");
...
function readCustomerFeeSettingsFromForm_(formEl) {
  const rows = [];
  const legacy = { parking_fee_amount: 0, travel_fee_amount: 0, key_pickup_fee_rule: "free", key_return_fee_rule: "free" };
  CUSTOMER_FEE_FIELDS_.forEach((field) => {
    const role = field.role;
    ...
    if (mode === "master_price") {
      ...
      const amount = Math.max(0, Number(parts[1] || 0) || 0);
      if (!rid) return;
      ...
      if (role === "parking") legacy.parking_fee_amount = amount;
      if (role === "business_trip") legacy.travel_fee_amount = amount;
      if (role === "key_pickup") legacy.key_pickup_fee_rule = "paid";
      if (role === "key_return") legacy.key_return_fee_rule = "paid";
      return;
    }
...
export async function renderMeetingCustomerForm(appEl, query) {
    ...
    <form data-el="meetingCustomerForm">
      <div class="card" style="margin-top:12px;">
        ...
        <div class="p" style="margin-top:8px;">
          ...
          ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[0], customerFeeOptions.parking, c.parking_fee_amount || c.parkingFeeAmount || 0)}
          ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[1], customerFeeOptions.business_trip, c.travel_fee_amount || c.travelFeeAmount || 0)}
          ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[2], customerFeeOptions.travel, 0)}
          ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[3], customerFeeOptions.key_pickup, 0, kpfr === "paid")}
          ${customerFeeFieldHtml_(CUSTOMER_FEE_FIELDS_[4], customerFeeOptions.key_return, 0, krfr === "paid")}
          <div class="p" style="margin-bottom:10px;">
  ...
  saveBtn?.addEventListener("click", async () => {
    ...
    const customer = {
      ...
      key_pickup_fee_rule: feeSettings.legacy.key_pickup_fee_rule,
      key_return_rule: getFormValue_(formEl, "key_return_rule"),
      key_return_rule_other: getFormValue_(formEl, "key_return_rule_other"),
      key_return_fee_rule: feeSettings.legacy.key_return_fee_rule,
      ...
      parking_fee_amount: String(feeSettings.legacy.parking_fee_amount),
      travel_fee_amount: String(feeSettings.legacy.travel_fee_amount),
