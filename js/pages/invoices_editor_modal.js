...
export async function inputCancellationFeeRateByModal_(current = {}) {
  const sourceAmount = Math.max(0, Number(current?.source_amount || 0) || 0);
  const rateType = String(current?.rate_type || "50").trim() || "50";
  const finalAmount = Math.max(0, Number(current?.final_amount || current?.amount || 0) || 0);
  const reason = String(current?.manual_reason || "").trim();
  const out = await openEditorModal_({
    title: "キャンセル料金",
    bodyHtml: `
      <div style="display:grid; gap:8px;">
        <div class="p">算出元: ${escapeHtml(formatMoney_(sourceAmount))}円</div>
        <div>
          <div class="label-sm">料率</div>
          <select class="input" data-el="rate">
            <option value="100" ${rateType === "100" ? "selected" : ""}>100%</option>
            <option value="50" ${rateType === "50" ? "selected" : ""}>50%</option>
            <option value="manual" ${rateType === "manual" ? "selected" : ""}>自由入力</option>
          </select>
        </div>
        <div>
          <div class="label-sm">請求額</div>
          <input class="input text-right" data-el="amount" type="number" min="0" step="1" inputmode="numeric" value="${escapeHtml(String(finalAmount || Math.round(sourceAmount * 0.5)))}" />
        </div>
        <div data-el="reasonWrap">
          <div class="label-sm">自由入力理由</div>
          <input class="input" data-el="reason" type="text" value="${escapeHtml(reason)}" />
        </div>
      </div>
    `,
    onOpen: (root) => {
      const rateEl = root.querySelector('[data-el="rate"]');
      const amountEl = root.querySelector('[data-el="amount"]');
      const reasonWrap = root.querySelector('[data-el="reasonWrap"]');
      const sync_ = () => {
        const rate = String(rateEl?.value || "").trim();
        if (amountEl && rate === "100") amountEl.value = String(Math.round(sourceAmount));
        if (amountEl && rate === "50") amountEl.value = String(Math.round(sourceAmount * 0.5));
        if (amountEl) amountEl.readOnly = rate !== "manual";
        if (reasonWrap) reasonWrap.style.display = rate === "manual" ? "" : "none";
      };
      rateEl?.addEventListener("change", sync_);
      sync_();
    },
    onSubmit: (root) => {
      const nextRateType = String(root.querySelector('[data-el="rate"]')?.value || "").trim();
      const nextAmount = Math.max(0, Number(root.querySelector('[data-el="amount"]')?.value || 0) || 0);
      const nextReason = String(root.querySelector('[data-el="reason"]')?.value || "").trim();
      if (!["100", "50", "manual"].includes(nextRateType)) return null;
      if (!(nextAmount > 0)) return null;
      if (nextRateType === "manual" && !nextReason) return null;
      return {
        rate_type: nextRateType,
        source_amount: sourceAmount,
        final_amount: Math.round(nextAmount),
        manual_reason: nextRateType === "manual" ? nextReason : "",
      };
    }
  });
  return out == null ? null : out;
