...
import { renderCustomerCreate, renderCustomersList } from "./pages/customers_list.js";
...
import { renderProductsPage, renderSettings } from "./pages/settings.js";
...
async function route() {
  ...
  else if (path.startsWith("/settings") || path.startsWith("/products")) setActiveNav("settings");
  ...
  try {
    if (path === "/visits") {
      ...
      await renderTabWithLoading_("顧客登録画面を準備しています", () => renderCustomerCreate(app));
      ...
      await renderTabWithLoading_("商品を読み込んでいます", () => renderProductsPage(app, query));
