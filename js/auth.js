...
const KEY_ACTIVE_STORE_ID = "mf_active_store_id";
const KEY_ACTIVE_STORE_NAME = "mf_active_store_name";
const KEY_AUTH_CLIENT_TRACE_ID = "mf_auth_client_trace_id";
...
function _newAuthClientTraceId_() {
  try {
    if (window.crypto && typeof window.crypto.randomUUID === "function") return `act_${window.crypto.randomUUID()}`;
  } catch (_) {}
  return `act_${Date.now()}_${Math.random().toString(16).slice(2)}`;
}

function _getAuthClientTraceId_() {
  try {
    const existing = String(sessionStorage.getItem(KEY_AUTH_CLIENT_TRACE_ID) || "").trim();
    if (existing) return existing;
    const next = _newAuthClientTraceId_();
    sessionStorage.setItem(KEY_AUTH_CLIENT_TRACE_ID, next);
    return next;
  } catch (_) {
    return _newAuthClientTraceId_();
  }
}
...
export function getUser() {
  if (_user) return _applyActiveStoreContext_(_user);
  try {
    const raw = sessionStorage.getItem(KEY_USER);
    if (!raw) return null;
    _user = JSON.parse(raw);
    return _applyActiveStoreContext_(_user);
  } catch (e) {
    return null;
  }
}
...
function _authClientLogMeta_() {
  return {
    ...
    client_trace_id: _getAuthClientTraceId_(),
...
function _logPasskeyLoginClientEvent_(phase, meta = {}, errorName = "", errorMessage = "") {
  _sendPasskeyClientLog_("", {
    phase: String(phase || "passkey_login_unknown"),
    error_name: String(errorName || ""),
    error_message: String(errorMessage || ""),
    meta: Object.assign({}, _authClientLogMeta_(), meta || {}),
  });
...
function _getPasskeyBrowserSupport_() {
  const ua = String(navigator.userAgent || "");
  const inAppRules = [
    { name: "Google app", re: /\bGSA\//i },
    { name: "LINE", re: /\bLine\//i },
    { name: "Facebook", re: /\bFBAN\/|\bFBAV\//i },
    { name: "Instagram", re: /\bInstagram\b/i },
    { name: "X", re: /\bTwitter\b/i },
    { name: "WeChat", re: /\bMicroMessenger\//i },
  ];
  const matched = inAppRules.find((rule) => rule.re.test(ua));
  const hasPublicKeyCredential = !!window.PublicKeyCredential;
  const hasNavigatorCredentials = !!navigator.credentials;
  const isSecureContext = !!window.isSecureContext;
  const isInAppBrowser = !!matched;
  const canCreatePasskey = hasPublicKeyCredential && hasNavigatorCredentials && isSecureContext && !isInAppBrowser;
  let reason = "";
  if (isInAppBrowser) reason = "in_app_browser";
  else if (!hasPublicKeyCredential || !hasNavigatorCredentials) reason = "webauthn_unavailable";
  else if (!isSecureContext) reason = "insecure_context";
  return {
    can_create_passkey: canCreatePasskey,
    reason,
    in_app_browser: isInAppBrowser,
    in_app_browser_name: matched ? matched.name : "",
    has_public_key_credential: hasPublicKeyCredential,
    has_navigator_credentials: hasNavigatorCredentials,
...
async function _ensurePasskeyRegistered_(authToken, meRes, opts = {}) {
  ...
  if (hasPasskey && !force) return true;
  if (!window.PublicKeyCredential || !navigator.credentials || !window.isSecureContext) {
    await _sendPasskeyClientLog_(authToken, {
      phase: "register_capability_unavailable",
      error_name: "UnsupportedWebAuthn",
      error_message: "",
      meta: Object.assign({}, _authClientLogMeta_(), {
        has_public_key_credential: !!window.PublicKeyCredential,
        has_navigator_credentials: !!navigator.credentials,
      }),
    });
    return false;
  }
  ...
  await _sendPasskeyClientLog_(authToken, {
    phase: shouldRegister ? "register_confirm_accepted" : "register_confirm_cancelled",
    error_name: "",
    error_message: "",
    meta: _authClientLogMeta_(),
  });
...
async function _ensurePasskeyRegisteredWithTap_(authToken, meRes, container) {
  ...
  const passkeyEnv = _getPasskeyBrowserSupport_();
  const showBrowserGuide = !passkeyEnv.can_create_passkey;
  const browserGuide = passkeyEnv.in_app_browser
    ? `${passkeyEnv.in_app_browser_name}内のブラウザではパスキー登録できない可能性があります。SafariまたはChromeアプリで開いてください。`
    : "このブラウザではパスキー登録に必要な機能を利用できません。SafariまたはChromeアプリで開いてください。";
  host.innerHTML = `
    <section class="section">
      <div style="position:relative;min-height:58vh;">
        ...
        <div style="position:absolute;left:0;right:0;top:50%;transform:translateY(-50%);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;">
          ...
          ${showBrowserGuide ? `
            <p class="p" style="text-align:center;margin:0;max-width:320px;">${_escapeHtml_(browserGuide)}</p>
            <button id="btnPasskeyCopyUrl" class="btn" type="button">URLをコピー</button>
  ...
  await new Promise((resolve) => {
    ...
    let done = false;
    let screenRemovedLogged = false;
    const logTapEvent = (phase, meta = {}, errorName = "", errorMessage = "") => {
      _sendPasskeyClientLog_(authToken, {
        phase,
        error_name: String(errorName || ""),
        error_message: String(errorMessage || ""),
        meta: Object.assign({}, _authClientLogMeta_(), {
          passkey_browser: passkeyEnv,
          already_registered: alreadyRegistered,
          elapsed_ms: Date.now() - startedAt,
          has_host: !!host,
          setup_button_present: !!document.getElementById("btnPasskeySetup"),
          copy_url_button_present: !!document.getElementById("btnPasskeyCopyUrl"),
          skip_button_present: !!document.getElementById("btnPasskeySkip"),
        }, meta || {}),
      });
    };
    const cleanup = () => {
    ...
    const finish = (reason) => {
      ...
      logTapEvent("register_tap_resolved", { reason: String(reason || "") });
      cleanup();
      resolve();
    };
    const onHashChange = () => logTapEvent("register_tap_hashchange", { hash: String(location.hash || "") });
    const onPageHide = (event) => logTapEvent("register_tap_pagehide", { persisted: !!event.persisted });
    const onVisibilityChange = () => logTapEvent("register_tap_visibilitychange");
    const observer = new MutationObserver(() => {
      if (done) return;
      if (document.getElementById("btnPasskeySetup") || document.getElementById("btnPasskeySkip")) return;
      if (screenRemovedLogged) return;
      screenRemovedLogged = true;
      logTapEvent("register_tap_screen_removed_unexpected", {
        host_connected: !!host.isConnected,
    ...
    logTapEvent(showBrowserGuide ? "register_browser_guidance_shown" : "register_tap_screen_shown");
    const copyUrlBtn = document.getElementById("btnPasskeyCopyUrl");
    if (copyUrlBtn) {
      copyUrlBtn.addEventListener("click", async () => {
        const url = String(location.href || "https://staff.pet-mafamille.com/");
        try {
          await navigator.clipboard.writeText(url);
          logTapEvent("register_browser_guidance_url_copied");
          toast({ title: "コピーしました", message: "SafariまたはChromeに貼り付けて開いてください。" });
        } catch (e) {
          logTapEvent("register_browser_guidance_url_copy_failed", {}, String(e?.name || "Error"), String(e?.message || ""));
          window.prompt("このURLをコピーしてSafariまたはChromeで開いてください。", url);
        }
      });
    }
    if (setupBtn) {
      setupBtn.addEventListener("click", async () => {
        logTapEvent("register_tap_setup_clicked");
        let registered = false;
        try {
          registered = await _ensurePasskeyRegistered_(authToken, meRes, { force: alreadyRegistered });
        } catch (e) {
          console.error("[passkey/register/tap] failed", e);
          const msg = String(e?.message || e || "").trim();
          logTapEvent("register_tap_setup_failed", {}, String(e?.name || "Error"), msg);
          toast({ title: "パスキー登録未完了", message: msg || "パスキー登録に失敗しました。" });
        }
        logTapEvent(registered ? "register_tap_setup_completed" : "register_tap_setup_not_completed");
        if (registered) finish("setup");
      }, { once: true });
    }
    if (skipBtn) {
      skipBtn.addEventListener("click", () => {
        logTapEvent("register_tap_skip_clicked");
        const ok = window.confirm("パスキー登録はできるだけ早急に実施してください。今回はスキップしますか？");
        logTapEvent(ok ? "register_tap_skip_confirmed" : "register_tap_skip_cancelled");
        if (ok) finish("skip");
      });
    }
  });
...
async function _loginWithPasskey_() {
  if (!window.PublicKeyCredential || !navigator.credentials || !window.isSecureContext) {
    _logPasskeyLoginClientEvent_("login_capability_unavailable", {
      has_public_key_credential: !!window.PublicKeyCredential,
      has_navigator_credentials: !!navigator.credentials,
    }, "UnsupportedWebAuthn", "");
  ...
  if (typeof getOptions !== "function" || typeof verify !== "function") {
    _logPasskeyLoginClientEvent_("login_api_unavailable");
    throw new Error("passkey login api unavailable");
  }
  let optionsRes = null;
  try {
    optionsRes = await getOptions({ client_trace_id: _getAuthClientTraceId_() });
  } catch (e) {
    _logPasskeyLoginClientEvent_("login_options", {}, String(e?.name || "Error"), String(e?.message || ""));
    throw e;
  }
  const challengeId = String(optionsRes?.challenge_id || "").trim();
  const options = optionsRes && optionsRes.options;
  if (!challengeId || !options) {
    _logPasskeyLoginClientEvent_("login_options_invalid");
    throw new Error("passkey options invalid");
  }
  const publicKey = _normalizeRequestOptions_(options);
  let assertion = null;
  try {
    _logPasskeyLoginClientEvent_("login_get_start", {
      challenge_id: challengeId,
      allow_credentials_count: Array.isArray(publicKey.allowCredentials) ? publicKey.allowCredentials.length : 0,
    });
    assertion = await navigator.credentials.get({ publicKey });
  } catch (e) {
    _logPasskeyLoginClientEvent_("login_get", {
      has_focus: !!document.hasFocus(),
      hidden: !!document.hidden,
    }, String(e?.name || "Error"), String(e?.message || ""));
    throw e;
  }
  if (!assertion) {
    _logPasskeyLoginClientEvent_("login_get_empty", { challenge_id: challengeId });
    throw new Error("passkey assertion canceled");
  }
  const credential = _serializeCredential_(assertion);
  let verifyRes = null;
  try {
    verifyRes = await verify({ challenge_id: challengeId, credential, client_trace_id: _getAuthClientTraceId_() });
  } catch (e) {
    _logPasskeyLoginClientEvent_("login_verify", {}, String(e?.name || "Error"), String(e?.message || ""));
    throw e;
  }
  if (!verifyRes || verifyRes.success === false || !verifyRes.token) {
    _logPasskeyLoginClientEvent_("login_verify_rejected", {}, "PasskeyVerifyRejected", String((verifyRes && (verifyRes.error || verifyRes.message)) || ""));
...
export function initGoogleLogin({ containerId = "app", onLogin } = {}) {
  ...
  let manualPasskeyRetryRunning = false;
    ...
    container.innerHTML = `
      <section class="section">
        ...
        <div style="display:flex;justify-content:center;margin-top:14px;">
          <button id="manualPasskeyRetryBtn" class="btn" type="button">パスキーでログイン</button>
        </div>
      </section>
    `;
    const retryBtn = document.getElementById("manualPasskeyRetryBtn");
    if (retryBtn) {
      retryBtn.addEventListener("click", async () => {
        if (manualPasskeyRetryRunning) return;
        manualPasskeyRetryRunning = true;
        retryBtn.disabled = true;
        _logPasskeyLoginClientEvent_("login_manual_retry_start");
        try {
          await _loginWithPasskey_();
        } catch (e) {
          manualPasskeyRetryRunning = false;
          retryBtn.disabled = false;
          _logPasskeyLoginClientEvent_("login_manual_retry_failed", {}, String(e?.name || "Error"), String(e?.message || ""));
          toast({ title: "ログイン失敗", message: String(e?.message || "パスキー認証に失敗しました。") });
        }
      });
    }
  }
  ...
  (async () => {
    ...
    try {
      ...
      if (String(e?.message || "") === "passkey auto timeout") {
        _logPasskeyLoginClientEvent_("login_auto_timeout", { timeout_ms: AUTO_PASSKEY_TIMEOUT_MS });
      }
      await renderGisRecoveryScreen_();
    }
    autoRun.catch((e) => {
...
export function setUser(user) {
  _user = user || null;
  _syncActiveStoreIdWithUser_(_user);
  try {
...
function _getActiveStoreId_() {
  try {
    return String(localStorage.getItem(KEY_ACTIVE_STORE_ID) || "").trim();
  } catch (_) {
    return "";
  }
}

function _setActiveStoreId_(storeId) {
  const next = String(storeId || "").trim();
  try {
    if (next) localStorage.setItem(KEY_ACTIVE_STORE_ID, next);
    else localStorage.removeItem(KEY_ACTIVE_STORE_ID);
  } catch (_) {
  }
}

function _getActiveStoreName_() {
  try {
    return String(localStorage.getItem(KEY_ACTIVE_STORE_NAME) || "").trim();
  } catch (_) {
    return "";
  }
}

function _setActiveStoreName_(storeName) {
  const next = String(storeName || "").trim();
  try {
    if (next) localStorage.setItem(KEY_ACTIVE_STORE_NAME, next);
    else localStorage.removeItem(KEY_ACTIVE_STORE_NAME);
  } catch (_) {
  }
}

function _syncActiveStoreIdWithUser_(user) {
  const u = (user && typeof user === "object") ? user : null;
  if (!u) {
    _setActiveStoreId_("");
    return;
  }
  const current = _getActiveStoreId_();
  if (current) return;
  const fallback = String(u.store_id || u.org_id || "").trim();
  if (fallback) _setActiveStoreId_(fallback);
  const fallbackName = String(u.store_name || "").trim();
  if (fallbackName) _setActiveStoreName_(fallbackName);
}

function _applyActiveStoreContext_(user) {
  const u = (user && typeof user === "object") ? user : null;
  if (!u) return null;
  const activeStoreId = _getActiveStoreId_();
  if (!activeStoreId) return u;
  ...
  return Object.assign({}, u, {
    ...
    store_name: _getActiveStoreName_() || String(u.store_name || "").trim() || fromMembership,
  });
}

export function setActiveStoreContext_(storeId, storeName = "") {
  const next = String(storeId || "").trim();
  if (!next) return;
  const nextName = String(storeName || "").trim();
  _setActiveStoreId_(next);
  _setActiveStoreName_(nextName);
  if (_user && typeof _user === "object") {
    _user = Object.assign({}, _user, { store_id: next, org_id: next, store_name: nextName || _user.store_name || "" });
    try {
      sessionStorage.setItem(KEY_USER, JSON.stringify(_user));
    } catch (_) {
    }
  }
  window.dispatchEvent(new CustomEvent("mf:auth:changed", { detail: { authed: true } }));
...
export function syncActiveStoreName_(storeId, storeName) {
  const sid = String(storeId || "").trim();
  const sname = String(storeName || "").trim();
  if (!sid || !sname) return;
  const currentStoreId = _getActiveStoreId_();
  if (currentStoreId && currentStoreId !== sid) return;
  _setActiveStoreId_(sid);
  _setActiveStoreName_(sname);
  if (_user && typeof _user === "object") {
    _user = Object.assign({}, _user, { store_id: sid, org_id: sid, store_name: sname });
    try {
      sessionStorage.setItem(KEY_USER, JSON.stringify(_user));
    } catch (_) {
    }
